    ...
    partial class ZipEntry
    {
        ...
        internal static ZipEntry ReadDirEntry(System.IO.Stream s, System.Text.Encoding expectedEncoding)
        {
            ...
            unchecked
            {
                ...
                zde._LastModified = SC_CompressLib.Zip.SharedUtilities.PackedToDateTime(zde._TimeBlob);
            ...
            {
                // UTF-8 is in use
                zde._LocalFileName = SC_CompressLib.Zip.SharedUtilities.Utf8StringFromBuffer(block, block.Length);
            }
            else
            {
                zde._LocalFileName = SC_CompressLib.Zip.SharedUtilities.StringFromBuffer(block, block.Length, expectedEncoding);
            }
            ...
            if (zde._commentLength > 0)
            {
                ...
                {
                    // UTF-8 is in use
                    zde._Comment = SC_CompressLib.Zip.SharedUtilities.Utf8StringFromBuffer(block, block.Length);
                }
                else
                {
                    zde._Comment = SC_CompressLib.Zip.SharedUtilities.StringFromBuffer(block, block.Length, expectedEncoding);
                }
            }

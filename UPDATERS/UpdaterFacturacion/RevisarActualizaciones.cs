...
using UpdaterFacturacion;
using UpdaterFacturacion.Data;
using UpdaterFacturacion.FuncionesGenerales;

using Microsoft.VisualBasic;
using Microsoft.Win32;

namespace UpdaterFacturacion
{
    ...
    public class RevisarActualizaciones
    {
        ...
        {
            Exe = 1, Dll = 2, Reporte = 3, Excel = 4, HojasDeEstilo = 5, Otro = 99  
        }
        ...
        string sRutaDescarga_CFDI_Reportes = "";
        string sRutaDescarga_CFDI_Estilos = "";
        string sFullName = "";
        string sUrl = "";
        ...
        string sNombreLog = Application.StartupPath + "\\LogUpdaterFacturacion_"; 
        ...
        public RevisarActualizaciones(string Ruta, string Modulo, string ModuloDescarga, string Version)
        {
            sModulo = Modulo;
            sRutaDescarga = Ruta + @"\";
            sRutaDescarga_CFDI_Reportes = sRutaDescarga + @"\CFDI\Reportes\";
            sRutaDescarga_CFDI_Estilos = sRutaDescarga + @"\CFDI\Estilos\";
            ...
            if (!Directory.Exists(sRutaDescarga_CFDI_Reportes))
            {
                Directory.CreateDirectory(sRutaDescarga_CFDI_Reportes);
            }

            if (!Directory.Exists(sRutaDescarga_CFDI_Estilos))
            {
                Directory.CreateDirectory(sRutaDescarga_CFDI_Estilos);
            }
        } 
        ...
        private void CopiarArchivos()
        {
            ...
            foreach(string s in Directory.GetFiles(sRutaTemp) )
            {
                ...
                if (f.Name.ToLower() != sModulo.ToLower())
                {
                    ...
                    {
                        Copiar(s, f.Name, TipoArchivo.HojasDeEstilo);
                    }
                    else
                    {
            ...
            try
            {
                switch(TipoDeArchivo) 
                {
                    ...
                    case TipoArchivo.Reporte:
                        File.Copy(Archivo, Path.Combine(sRutaDescarga_CFDI_Reportes, FileName), true); 
                        break; 

                    case TipoArchivo.HojasDeEstilo:
                        File.Copy(Archivo, Path.Combine(sRutaDescarga_CFDI_Estilos, FileName), true); 
                        break; 
    ...
    public class LeerVersion : clsLeer
    {
        ...
        private UpdaterFacturacion.wsUpdater.wsUpdater conexion;
        ...
        {
            conexion = new UpdaterFacturacion.wsUpdater.wsUpdater();

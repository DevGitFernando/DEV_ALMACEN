...
namespace UpdateFarmacia
{
    ...
    public class RevisarActualizaciones
    {
        ...
        {
            Exe = 1, Dll = 2, Reporte = 3, Excel = 4, Fuente_TTF = 5, Fuente_OTF = 6, Otro = 7
        }
        ...
        private string sModuloFarmacia = Application.StartupPath + @"\Farmacia.exe";
        ...
        Dictionary<string, string> filesWebService = new Dictionary<string, string>();
        ...
        public RevisarActualizaciones(string Ruta, string Modulo, string ModuloDescarga, string Version, Label ControlAvance)
        {
            ...
            sFileSetup = sRutaSetup + @"\Farmacia.SII";
            
            sFileSetup = "Farmacia.SII";
            sFileSetup_Ext = "FarmaciaExt.SII";
            ...
            filesWebService.Add("wsDllWebService.dll".ToUpper(), "wsDllWebService.dll".ToUpper());
            filesWebService.Add("wsRhWebService.dll".ToUpper(), "wsRhWebService.dll".ToUpper());
            filesWebService.Add("wsWebDll_SII_IRE_SIGHO.dll".ToUpper(), "wsWebDll_SII_IRE_SIGHO.dll".ToUpper());
            filesWebService.Add("wsWebDll_SII_IRE_INTERMED.dll".ToUpper(), "wsWebDll_SII_IRE_INTERMED.dll".ToUpper()); 
            //filesWebService.Add("".ToUpper(), "".ToUpper());
        ...
        private void PrepararDirectorio_Descargas()
        {
            ...
            {
                Registro("Borrando Farmacia.SII anterior.");
        ...
        private bool BuscarUpdateFarmacia()
        {
            int iSolicitudes = 0;
            // bool bRegresa = false;
            string sSql = "";           
        ...
        private bool BuscarServidorLocal() 
        {
            ...
            if ( ObtenerUrlLocal() )
            {
                ...
                sSql = string.Format(" Select IdTerminal, Nombre, MAC_Address, EsServidor " +
                    " From CFGC_Terminales (NoLock) " +
                    " Where replace(MAC_Address, '-', '') in ( {0} )  and EsServidor = 1 ", sListaDeMAC_Address);

                try
                {
                    wsFarmacia1.wsCnnCliente cnnFarmacia_x = new UpdateFarmacia.wsFarmacia1.wsCnnCliente();
                    cnnFarmacia_x.Url = sUrlLocal;
                    ////Registro("URL Local :  " + sUrlLocal);
                    ////RegistrarCambios();
                    ...
                    ////Registro("Datos :  " + leerSvr.DatosConexion.CadenaConexion);
                    ////RegistrarCambios();
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            Registro(sMensaje + "MAC de servidor [" + sListaDeMAC_Address + "]");
        ...
        private bool GetWWW_Directorio(bool RegistrarLog)
        {
            ...
            try
            {
                ...
                wsFarmacia1.wsCnnCliente cnnFarmacia = new UpdateFarmacia.wsFarmacia1.wsCnnCliente();
        ...
        private bool ObtenerUrl()
        {
            ...
            bool bRegresa = false;
            string sXml = General.UnidadSO + ":\\FarmaciaPtoVta.xml";
        ...
        private void CopiarArchivos_Aux()
        {
            ...
            foreach(string s in Directory.GetFiles(sRutaTemp) )
            {
                ...
                if (!filesWebService.ContainsKey(sModulo.ToUpper())) 
                {
        ...
        private bool validarActualizacionIIS()
        {
            bool bRegresa = false;
            string sFileModulo = sRutaTemp; 


            //// Recorer
            foreach (KeyValuePair<string, string> item in filesWebService)
            {
                sFileModulo = Path.Combine(sRutaTemp, item.Value.ToLower());
        ...
        private void Copiar_ArchivosWebService()
        {
            ...
            bool bActualizarIIS = validarActualizacionIIS();
    ...
    public class LeerVersion : clsLeer
    {
        ...
        private UpdateFarmacia.wsUpdater.wsUpdater  conexion;
        private UpdateFarmacia.wsFarmacia1.wsCnnCliente cnnFarmacia; 
        ...
        {
            conexion = new UpdateFarmacia.wsUpdater.wsUpdater();
            cnnFarmacia = new UpdateFarmacia.wsFarmacia1.wsCnnCliente(); 

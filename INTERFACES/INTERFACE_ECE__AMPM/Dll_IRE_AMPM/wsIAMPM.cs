...
using Dll_IRE_AMPM.Informacion;
using Dll_IRE_AMPM.wsClases; 

namespace Dll_IRE_AMPM
{
    ...
    public class wsIAMPM
    {
        ...
        string sFileConexion = "SII_INT_IAMPM";
        ...
        /// <returns></returns>
        [WebMethod(Description = "Consulta de medicamentos")]
        public string BusquedaDeMedicamentos(string Referencia_IdClinica, string IdFarmacia, int TipoDeBusqueda, string CriterioBusqueda, string GUID)
        {
            ...
            try
            {
                ...
                sRegresa = consulta.Consultar(Referencia_IdClinica, IdFarmacia, TipoDeBusqueda, CriterioBusqueda);

            }
            catch (Exception ex)
            {
                respuesta.Estatus = 1;
                respuesta.Mensaje = "Error al accesar la base de datos";
                sRegresa = respuesta.GetResponse();
            }
        ...
        public string RecepcionDeRecetaElectronica(string Referencia_IdClinica, string IdFarmacia, string Informacion_XML, string GUID)
        {
            ResponseGeneral respuesta = new ResponseGeneral();
            TipoProcesoReceta tpProceso = TipoProcesoReceta.Ninguno;

            clsDatosConexion datosCnn = new clsDatosConexion(AbrirConexionEx(sFileConexion));
            clsGrabarError Error = new clsGrabarError(datosCnn, General.DatosApp, "RecepcionDeRecetaElectronica()");
            ...
            try
            {
                ...
                sRegresa = receta.Guardar(Referencia_IdClinica, IdFarmacia, Informacion_XML).GetResponse();
            }
            catch (Exception ex)
            {
                respuesta.Estatus = 1;
                ...
                Error.GrabarError(ex.Message, "RecepcionDeRecetaElectronica()");
        ...
        private void GetLocalConfigurationKey()
        {
            ...
            int lenWebServiceName = AppDomain.CurrentDomain.BaseDirectory.ToString().Length;
            filePath = BaseDir.Substring(0, lenWebServiceName) + "wsSII_INT_IAMPM.ini"; //"Config.ini"           

            sURL_wsSIADISSEP = sURL_wsSIADISSEP;
            if (!File.Exists(filePath))
            {
                return;
            }

...
--- Configurar JOB 
	...
	Set @sPrefijo = 'RP_001' 
	Set @sJob_Name = @sPrefijo + '__SND_CAT__Mediaccess__' + @Nombre_BD + '' 
	...
	Set @sRutaAplicacion = 'C:\SII_OFICINA_CENTRAL\JOB__MEDIACCESS\' + 'MA Servicios.exe'
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@sJob_Name, 
		...
		@owner_login_name=N'IntermedCom\Intermed', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

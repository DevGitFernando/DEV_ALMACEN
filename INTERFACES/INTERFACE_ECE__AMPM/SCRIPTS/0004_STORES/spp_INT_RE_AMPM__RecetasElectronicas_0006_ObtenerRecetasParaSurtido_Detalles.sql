-------------------------------------------------------------------------------------------------------------------------------------------------- 
If Exists ( Select * From Sysobjects (NoLock) Where Name = 'spp_INT_RE_AMPM__RecetasElectronicas_0006_ObtenerRecetasParaSurtido_Detalles' and xType = 'P' ) 
   Drop Proc spp_INT_RE_AMPM__RecetasElectronicas_0006_ObtenerRecetasParaSurtido_Detalles
Go--#SQL 

Create Proc spp_INT_RE_AMPM__RecetasElectronicas_0006_ObtenerRecetasParaSurtido_Detalles 
( 
	@IdEmpresa varchar(3) = '002', 
	@IdEstado varchar(2) = '09', 
	@IdFarmacia varchar(4) = '0011', 
	@IdCliente varchar(4) = '0002', 
	@IdSubCliente varchar(4) = '0001',
	@FolioInterface varchar(20) = '000000000033', 
	@IdPersonal varchar(4) = '0004' 
) 
With Encryption 
As 
Begin 
...
------------------------------------------------- OBTENER LA INFORMACION   
	...
	--Left Join INT_RE_SIGHO__RecetasElectronicas_0003_Diagnosticos D (NoLock) 
	--	On ( G.IdEmpresa = D.IdEmpresa and G.IdEstado = D.IdEstado and G.IdFarmacia = D.IdFarmacia and G.Folio = D.Folio ) 		
	...
	Select 
		EnCuadroBasicoFarmacia = 'SI',  
		I.CodigoEAN, I.CantidadRequerida - I.CantidadEntregada As CantidadRequerida, C.DescripcionClave, 	
		I.IdEmpresa, I.IdEstado, I.IdFarmacia, I.Folio, 0 as Relacioando, C.ClaveSSA
	Into #tmp_02_Medicamentos 
	From INT_AMPM__RecetasElectronicas_0004_Insumos I (NoLock) 	
	inner Join vw_Productos_CodigoEAN C (NoLock) On ( I.CodigoEAN = C.CodigoEAN ) 	
	Where 	Exists (Select * From #tmp_01_General E (NoLock)
					Where I.IdEmpresa = E.IdEmpresa and I.IdEstado = E.IdEstado and I.IdFarmacia = E.IdFarmacia and I.Folio = E.Secuenciador ) 	


	------	Claves relacionadas  
	--Update M Set ClaveSSA = C.ClaveSSA, Relacioando = 1  
	--From #tmp_02_Medicamentos M 
	--Inner Join vw_Relacion_ClavesSSA_Claves C (NoLock) 
	--	On ( C.IdEstado = @IdEstado and C.ClaveSSA_Relacionada = M.ClaveSSA and C.Status = 'A' )  
...
---		spp_INT_RE_SIGHO__RecetasElectronicas_0010_ObtenerRecetasParaSurtido_Detalles 			
	...
	Select 
		...
		@FechaVigencia_Inicia = convert(varchar(10), GetDate(), 120), 
		@FechaVigencia_Termina = convert(varchar(10), dateadd(yy, 2,GetDate()), 120), 
...
---		select top 10 * from INT_RE_SIGHO__RecetasElectronicas_0004_Insumos 

-------------------------------------------------------------------------------------------------------------------------------------------------- 
If Exists ( Select * From Sysobjects (NoLock) Where Name = 'spp_INT_SESEQ__CEDIS_0001_GenerarDocumento_SIAM__01_Ventas' and xType = 'P' ) 
   Drop Proc spp_INT_SESEQ__CEDIS_0001_GenerarDocumento_SIAM__01_Ventas
Go--#SQL 

Create Proc spp_INT_SESEQ__CEDIS_0001_GenerarDocumento_SIAM__01_Ventas 
( 
...
Declare 
	...
	Select *, FolioVenta as FolioReferencia, FechaRegistro as FechaReferencia   
	Into #tmp__FoliosDeVenta 
	From VentasEnc D (NoLock) 
	Where D.IdEmpresa = @IdEmpresa and D.IdEstado = @IdEstado and D.IdFarmacia = @IdFarmacia and D.FolioVenta = right('000000000000' + @Folio, 8)  
	...
	Begin 
		...
		Inner Join Pedidos_Cedis_Enc_Surtido P (NoLock) 
			On ( D.IdEmpresa = P.IdEmpresa and D.IdEstado = P.IdEstado and D.IdEstado = P.IdEstado and 'SV' + D.FolioVenta = P.FolioTransferenciaReferencia ) 
	...
	--		spp_INT_SESEQ__CEDIS_0001_GenerarDocumento_SIAM__01_Ventas  

	--------------------------------- INFORMACION  
	Select 
		distinct 
		V.FolioReferencia, V.FechaReferencia, 
		V.IdEmpresa, V.IdEstado, V.IdFarmacia, V.FolioVenta, V.IdCliente, V.IdSubCliente, V.IdPrograma, v.IdSubPrograma, I.IdBeneficiario, 
		...
		right(V.FolioVenta, 4) + '-' + cast(DATEPART(year, v.FechaRegistro) as varchar(10)) as Folio,  
		...
		cast(VL.CantidadVendida as int) as Cantidad_Base,    
		cast(VL.CantidadVendida as int) as Cantidad   
	Into #tmp_Detalles 
	From VentasDet D (NoLock) 
	Inner Join VentasDet_Lotes VL (NoLock) On ( D.IdEmpresa = VL.IdEmpresa and D.IdEstado = VL.IdEstado and D.IdEstado = VL.IdEstado and D.FolioVenta = VL.FolioVenta ) 
	...
	Inner Join FarmaciaProductos_CodigoEAN_Lotes L (NoLock) 
		On ( VL.IdEmpresa = L.IdEmpresa and VL.IdEstado = L.IdEstado and VL.IdFarmacia = L.IdFarmacia and VL.IdSubFarmacia = L.IdSubFarmacia

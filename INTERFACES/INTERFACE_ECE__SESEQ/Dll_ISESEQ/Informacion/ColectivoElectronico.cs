...
namespace Dll_ISESEQ.Informacion
{
    public class ColectivoElectronico
    {
        ...
        public ColectivoElectronico(clsDatosConexion DatosConexion)
        {
            ...
            Error = new clsGrabarError(datosDeConexion, GnDll_SII_SESEQ.DatosApp, "ColectivoElectronico");
        ...
        public DataSet PrepararInformacion(DataSet Informacion)
        {
            ...
            {
                leer.RenombrarTabla("Generales", "T01__General");
            }

            if (leer.ExisteTabla("Paciente"))
            {
                leer.RenombrarTabla("Paciente", "T02__ListaPacientes");
            }

            if (leer.ExisteTabla("Medicamentos"))
            {
                leer.RenombrarTabla("Medicamentos", "T03__PacientesMedicamentos");
            }

            if (leer.ExisteTabla("Medicamento"))
            {
                leer.RenombrarTabla("Medicamento", "T04__ListaMedicamentos");
            }

            if (leer.ExisteTabla("Soluciones"))
            {
                leer.RenombrarTabla("Soluciones", "T05__PacientesSoluciones");
            }

            if (leer.ExisteTabla("Solucion"))
            {
                leer.RenombrarTabla("Solucion", "T06__ListaSolucion");
            }
        ...
        public TipoProcesoReceta TipoDeDocumento(string Informacion_XML)
        {
            ...
            sFolio_SESEQ = datos.Campo("idPeticion");
        ...
        public ResponseRecetaElectronica Guardar(string Informacion_XML)
        {
            ...
            datos.DataTableClase = leerXML.Tabla("T01__General");
            datos.Leer();
            sIdUMedica = datos.Campo("unidadMedica");
            sIdUMedica = datos.Campo("idClues");
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (Guardar_001_XML(respuesta))
                    {
                        ...
                        respuesta.AddItemRespuesta("idPeticion", sFolio_SESEQ);
                        //respuesta.AddItemRespuesta("expediente", sExpediente_SESEQ);
                        respuesta.AddItemRespuesta("idColectivo", sFolio_SESEQ); //sFolioReceta_SESEQ); 
        ...
        private string ObtenerNombreUnidad()
        {
            ...
            string sSql = string.Format(
                "Select F.IdEstado, F.IdFarmacia, F.NombreFarmacia as Farmacia, E.Referencia_SESEQ " + 
                "From CatFarmacias F (NoLock) " +
                "Inner Join INT_SESEQ__CFG_Farmacias_UMedicas E (NoLock) On ( F.IdEstado = E.IdEstado and F.IdFarmacia = E.IdFarmacia ) " + 
                "Where Referencia_SESEQ = '{0}' ", sIdUMedica);
        ...
        private bool Guardar_002_General(ResponseRecetaElectronica Respuesta)
        {
            ...
            string sFolioColectivo = "";
            string sidTipoServicio = "";
            string sidServicio = "";
            ...
            clsLeer datosEncabezado = new clsLeer();
            clsLeer datos = new clsLeer();

            datosEncabezado.DataTableClase = leerXML.Tabla("T01__General");
            datos.DataTableClase = leerXML.Tabla("T02__ListaPacientes");
            bRegresa = datos.Registros > 0;

            datosEncabezado.Leer(); 
            while (datos.Leer())
            {
                sFolioReceta = string.Format("{0}-{1}", sFolio_SESEQ, datos.Campo("FolioPaciente"));
                sIdPaciente = datos.Campo("Paciente_Id"); 
                sSql = string.Format("Exec spp_INT_SESEQ__RecetasElectronicas_001_XML " +
                    " @UMedica = '{0}', @Folio_SESEQ = '{1}', @InformacionXML = '{2}', @TipoDeProceso = '{3}' ",
                    sIdUMedica, sFolioReceta, sInformacion_XML, (int)tpProceso);
                ...
                else
                {
                    ...
                    sFolioReceta = string.Format("{0}-{1}", sFolio_SESEQ, datos.Campo("FolioPaciente"));
                    sFechaReceta = GetFecha(datosEncabezado.Campo("fechaHoraElaboracion")) + " " + GetHora(datosEncabezado.Campo("fechaHoraElaboracion"));
                    sFechaEnvioReceta = GetFecha(datosEncabezado.Campo("fechaHoraElaboracion")) + " " + GetHora(datosEncabezado.Campo("fechaHoraElaboracion"));

                    sFolioColectivo = datosEncabezado.Campo("idColectivo");
                    sidTipoServicio = datosEncabezado.Campo("idTipoServicio");
                    sidServicio = datosEncabezado.Campo("idServicio");
                    sidEpisodio = datosEncabezado.Campo("idEpisodio");
                    sCamaPaciente = datos.Campo("camaPaciente");
                    datos.Campo("FolioPaciente");

                    sEsResurtible = datos.Campo("isResurtible"); ;
                    sNumeroDePartidas = datos.Campo("noResurtible");
                    sProgramaAtencion = datos.Campo("Programa");

                    sFolioAfiliacionSPSS = datos.Campo("polizaPaciente"); // folioAfiliacionSPSS
                    sRangoVigencia = datos.Campo("vigenciaPolizaPaciente");
                    ...
                    sExpediente = datos.Campo("noece");
                    sFolioPaciente = datos.Campo("folioPaciente");
                    sBeneficiarioNombre = datos.Campo("nombrePaciente");
                    sBeneficiarioApPaterno = datos.Campo("apPaternoPaciente");
                    sBeneficiarioApMaterno = datos.Campo("apMaternoPaciente");
                    sBeneficiarioSexo = "M"; // datos.Campo("sexo");
                    sFechaNacimiento = GetFecha(datos.Campo("fechaNacimientoPaciente"));
                    ...
                    sSql = string.Format("Exec spp_INT_SESEQ__RecetasElectronicas_0001_General " +
                        ...
                        " @FolioColectivo = '{23}', @idTipoServicio = '{24}', @idServicio = '{25}', @camaPaciente = '{26}', @idEpisodio = '{27}', @idPaciente = '{28}'",
                        ...
                        sFolioColectivo, sidTipoServicio, sidServicio, sCamaPaciente, sidEpisodio, sFolioPaciente
                        );
                    ...
                    if (bRegresa)
                    {
                        bRegresa = Guardar_005_Medicamentos(Respuesta, sIdPaciente);
                    }

                    if (bRegresa)
                    {
                        bRegresa = Guardar_006_Soluciones(Respuesta, sIdPaciente);
                    }
        ...
        private bool Guardar_005_Medicamentos(ResponseRecetaElectronica Respuesta, string IdPaciente)
        {
            bool bRegresa = true;
            string sSql = "";
            string sClaveSSA = "";
            int iCantidad = 0;
            string sMedicamentos_Id = ""; 

            clsLeer datos = new clsLeer();
            clsLeer datosPacientes = new clsLeer();

            datosPacientes.DataTableClase = leerXML.Tabla("T03__PacientesMedicamentos");
            try
            {
                datosPacientes.DataRowsClase = datosPacientes.DataTableClase.Select(string.Format(" Paciente_Id = '{0}' ", IdPaciente) );
                datosPacientes.Leer();
                sMedicamentos_Id = datosPacientes.Campo("Medicamentos_Id");

                datos.DataTableClase = leerXML.Tabla("T04__ListaMedicamentos");
                datos.DataRowsClase = datos.DataTableClase.Select(string.Format(" Medicamentos_Id = '{0}' ", sMedicamentos_Id));
                
            }
            catch
            {
                datos = new clsLeer();
                datosPacientes = new clsLeer();
            }
        ...
        private bool Guardar_006_Soluciones(ResponseRecetaElectronica Respuesta, string IdPaciente)
        {
            bool bRegresa = true;
            string sSql = "";
            string sClaveSSA = "";
            ...
            datosPacientes.DataTableClase = leerXML.Tabla("T05__PacientesSoluciones");
            try
            {
                datosPacientes.DataRowsClase = datosPacientes.DataTableClase.Select(string.Format(" Paciente_Id = '{0}' ", IdPaciente));
                datosPacientes.Leer();
                sMedicamentos_Id = datosPacientes.Campo("Soluciones_Id");

                datos.DataTableClase = leerXML.Tabla("T06__ListaSolucion");
                datos.DataRowsClase = datos.DataTableClase.Select(string.Format(" Soluciones_Id = '{0}' ", sMedicamentos_Id));

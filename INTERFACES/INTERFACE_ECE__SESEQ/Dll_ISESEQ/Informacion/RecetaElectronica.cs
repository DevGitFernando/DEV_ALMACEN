using System;
using System.Collections;
using System.Collections.Generic; 
using System.Data;
using System.Configuration;
using System.Web;
using System.Web.Services;
using System.Web.Services.Protocols;
using System.Collections;
using System.ComponentModel;
using System.Diagnostics;
using System.Xml;
using System.Data.SqlClient;
using System.Data.SqlTypes;
using System.IO;
using System.Reflection;
using System.Security.Cryptography;
using System.Data.Odbc;

using Microsoft.VisualBasic;

using System.Text;
using System.IO;
using System.Configuration;

using SC_SolutionsSystem;
using SC_SolutionsSystem.Data;
using SC_SolutionsSystem.Errores;
using SC_SolutionsSystem.Reportes;
using SC_SolutionsSystem.FuncionesGenerales;
using SC_SolutionsSystem.FuncionesGrid;

using Dll_ISESEQ;
using Dll_ISESEQ.wsClases;

namespace Dll_ISESEQ.Informacion
{
    public class RecetaElectronica
    {
        clsDatosConexion datosDeConexion;
        clsConexionSQL cnn;
        clsLeer leer;
        clsLeer leerXML;
        clsGrabarError Error;

        string sIdEmpresa = "";
        string sIdEstado = "";
        string sIdFarmacia = "";

        string sNombre_CampoFolio = "";
        string sNombre_CampoFechaReceta = "";
        string sNombre_CampoFechaEnvio = "";

        string sIdUMedica = "";
        string sInformacion_XML = "";
        string sInformacion_XML_Converted = "";
        string sFolio_SESEQ = "";
        string sExpediente_SESEQ = "";
        string sFolioReceta_SESEQ = ""; 
        string sFolioRegistro = "";
        string sFolioRegistro_XML = "";
        string sMensajesError = "";
        string sTipoDeProceso = "";
        string sCLUES = "";
        int iRecepcionPrevia = 0; 


        DataSet dtsInformacion = new DataSet();

        basGenerales Fg = new basGenerales();
        string sEncabezado = "";
        Dictionary<TipoProcesoReceta, TipoProcesoReceta> listaProcesos = new Dictionary<TipoProcesoReceta, TipoProcesoReceta>();
        TipoProcesoReceta proceso = TipoProcesoReceta.Ninguno;
        TipoProcesoReceta tpProceso = TipoProcesoReceta.Ninguno;

        #region Constructor de Clase 
        public RecetaElectronica(clsDatosConexion DatosConexion)
        {
            datosDeConexion = DatosConexion;
            cnn = new clsConexionSQL(datosDeConexion);
            leer = new clsLeer(ref cnn);
            leerXML = new clsLeer();

            Error = new clsGrabarError(datosDeConexion, GnDll_SII_SESEQ.DatosApp, "RecetaElectronica");
            Error.NombreLogErorres = "INT_SESEQ__CtlErrores";

            sEncabezado = string.Format("<?xml version={0}1.0{0} encoding={0}UTF-8{0}?>", Fg.Comillas());


            listaProcesos = new Dictionary<TipoProcesoReceta, TipoProcesoReceta>();
            listaProcesos.Add(TipoProcesoReceta.Receta, TipoProcesoReceta.Receta);
            listaProcesos.Add(TipoProcesoReceta.Colectivo, TipoProcesoReceta.Colectivo);

        }
        #endregion Constructor de Clase

        #region Propiedades Publicas
        public string CLUES_Default
        {
            get { return sCLUES; }
            set { sCLUES = value; }
        }
        #endregion Propiedades Publicas

        #region Funciones y Procedimientos Publicos
        public string toUTF8(string Cadena)
        {
            UTF8Encoding encoding = new UTF8Encoding();
            byte[] bytes = encoding.GetBytes(Cadena);
            return encoding.GetString(bytes);
        }

        public string Normalizar(string Cadena)
        {
            string sRegresa = Cadena; 

            string consignos = "áàäéèëíìïóòöúùuñÁÀÄÉÈËÍÌÏÓÒÖÚÙÜÑçÇ";
            string sinsignos = "aaaeeeiiiooouuunAAAEEEIIIOOOUUUNcC";

            if (Cadena != "")
            {
                for (int i = 0; i <= consignos.Length - 1; i++)
                {
                    sRegresa = sRegresa.Replace(consignos[i], sinsignos[i]);
                }
            }

            return sRegresa; 
        }

        public DataSet PrepararInformacion(DataSet Informacion)
        {
            DataSet dtsRetorno = new DataSet();
            clsLeer leer = new clsLeer();

            leer.DataSetClase = Informacion;

            if (leer.ExisteTabla("Generales"))
            {
                leer.RenombrarTabla("Generales", "General"); 
            }

            return leer.DataSetClase.Copy(); 
        }

        public TipoProcesoReceta TipoDeDocumento(string Informacion_XML)
        {
            //TipoProcesoReceta proceso = TipoProcesoReceta.Ninguno;
            proceso = TipoProcesoReceta.Ninguno;
            clsLeer datos = new clsLeer();

            XmlDocument xDoc = new XmlDocument();
            xDoc = Newtonsoft.Json.JsonConvert.DeserializeXmlNode(Informacion_XML, "DtsInformacion");
            sInformacion_XML_Converted = xDoc.InnerXml; 


            Informacion_XML = toUTF8(Fg.QuitarSaltoDeLinea(Informacion_XML.Replace("\t", "")));
            dtsInformacion = new DataSet();
            //dtsInformacion.ReadXml(new XmlTextReader(new StringReader(Informacion_XML)));
            dtsInformacion.ReadXml(new XmlTextReader(new StringReader(xDoc.InnerXml)));
            sInformacion_XML_Converted = dtsInformacion.GetXml(); 


            dtsInformacion = PrepararInformacion(dtsInformacion); 

            Informacion_XML = Normalizar(Informacion_XML);
            leerXML.DataSetClase = dtsInformacion;
            sInformacion_XML = Informacion_XML.Replace(sEncabezado, "");


            datos.DataTableClase = leerXML.Tabla("General");
            datos.Leer();

            sTipoDeProceso = datos.Campo("Origen").ToUpper();
            sTipoDeProceso = sTipoDeProceso == "" ? datos.Campo("TipoDePeticion") : sTipoDeProceso;

           
            if (sTipoDeProceso == TipoProcesoReceta.Receta.ToString().ToUpper()) proceso = TipoProcesoReceta.Receta;
            if (sTipoDeProceso == TipoProcesoReceta.Colectivo.ToString().ToUpper()) proceso = TipoProcesoReceta.Colectivo;


            if ( (TipoProcesoReceta)Convert.ToInt32("0"+ sTipoDeProceso ) == TipoProcesoReceta.Receta ) proceso = TipoProcesoReceta.Receta;
            if ( (TipoProcesoReceta)Convert.ToInt32("0" + sTipoDeProceso) == TipoProcesoReceta.Colectivo) proceso = TipoProcesoReceta.Colectivo;


            sNombre_CampoFolio = "FolioDeReceta";
            sNombre_CampoFechaReceta = "FechaDeReceta";
            sNombre_CampoFechaEnvio = "fechaEnvio";

            if (proceso == TipoProcesoReceta.Colectivo || proceso == TipoProcesoReceta.ColectivoMedicamentos)
            {
                sNombre_CampoFolio = "Folio";
                sNombre_CampoFechaReceta = "fechaSolicitud";
                sNombre_CampoFechaEnvio = "fechaSolicitud";
            }

            sIdUMedica = datos.Campo("claveUnidadMedica");
            sFolio_SESEQ = datos.Campo(sNombre_CampoFolio);

            return proceso; 
        }

        public ResponseRecetaElectronica Guardar(string Informacion_XML)
        {
            ResponseRecetaElectronica respuesta = new ResponseRecetaElectronica();
            clsLeer datos = new clsLeer();
            tpProceso = TipoProcesoReceta.SurteReceta;

            XmlDocument xDoc = new XmlDocument();
            xDoc = Newtonsoft.Json.JsonConvert.DeserializeXmlNode(Informacion_XML, "DtsInformacion");
            sInformacion_XML_Converted = xDoc.InnerXml; 


            Informacion_XML = toUTF8(Fg.QuitarSaltoDeLinea(Informacion_XML.Replace("\t", ""))); 
            dtsInformacion = new DataSet();
            //dtsInformacion.ReadXml(new XmlTextReader(new StringReader(Informacion_XML)));
            dtsInformacion.ReadXml(new XmlTextReader(new StringReader(xDoc.InnerXml)));

            dtsInformacion = PrepararInformacion(dtsInformacion); 

            Informacion_XML = Normalizar(Informacion_XML); 
            leerXML.DataSetClase = dtsInformacion; 
            sInformacion_XML = Informacion_XML.Replace(sEncabezado, "");


            datos.DataTableClase = leerXML.Tabla("General");
            datos.Leer();

            sIdUMedica = datos.Campo("claveUnidadMedica");
            sFolio_SESEQ = datos.Campo("FolioDeReceta");
            sFolio_SESEQ = datos.Campo(sNombre_CampoFolio);
            sTipoDeProceso = datos.Campo("Origen");
            sTipoDeProceso = sTipoDeProceso == "" ? datos.Campo("TipoDePeticion") : sTipoDeProceso;

            //// Se toma el valor leido al validar el tipo de requerimiento 
            tpProceso = proceso;


            //if (tpProceso.ToString().ToUpper() != sTipoDeProceso.ToUpper())
            if (!listaProcesos.ContainsKey(proceso))
            {
                respuesta.Estatus = 101;
                respuesta.Error = "Tipo de petición no valida para el proceso solicitado.";
            }
            else
            {
                sIdEmpresa = "";
                sIdEstado = "";
                sIdFarmacia = "";
                sFolioRegistro = "";
                sFolioRegistro_XML = "";
                Guardar_001_XML__Log(respuesta);

                try
                {

                    if (!cnn.Abrir())
                    {
                        respuesta.Estatus = 1;
                        respuesta.Error = "Error de conexión con el servidor de datos.";
                    }
                    else
                    {
                        cnn.IniciarTransaccion();

                        respuesta.Estatus = 200;


                        if (Guardar_001_XML(respuesta))
                        {
                            respuesta.Estatus = 100;
                            respuesta.ResetItemsRespuesta();
                            respuesta.AddItemRespuesta("FolioDeReceta", sFolio_SESEQ);
                            //respuesta.AddItemRespuesta("expediente", sExpediente_SESEQ);
                            //respuesta.AddItemRespuesta("FolioDeReceta", sFolioReceta_SESEQ); 

                            respuesta.Error = "Información integrada correctamente.";
                            cnn.CompletarTransaccion();
                        }
                        else
                        {
                            //respuesta.Estatus = 199;
                            Error.GrabarError(leer, "");
                            cnn.DeshacerTransaccion();
                        }

                        cnn.Cerrar();
                    }
                }
                catch
                {
                    Error.GrabarError(leer, "");
                    cnn.DeshacerTransaccion();
                    cnn.Cerrar();
                }
            }

            return respuesta; 
        }
        #endregion Funciones y Procedimientos Publicos
        
        #region Funciones y Procedimientos
        private string GetFechaIniciaVigencia(string Valor)
        {
            string sRegresa = "";

            try
            {
                sRegresa = General.FechaYMD(Convert.ToDateTime(Valor).AddYears(1*-3)); 
            }
            catch 
            { 
            }

            return sRegresa; 
        }

        private string GetFecha(string Valor)
        {
            string sRegresa = "";
            string[] sFecha = null; // Valor.Split('/');
            int EqualPosition = 0;

            try
            {
                Valor = Fg.Left(Valor, 10);
                //sFecha = Valor.Split('/');

                if (Valor.Contains("/") || Valor.Contains("-"))
                {
                    if (Valor.Contains("/"))
                    {
                        sFecha = Valor.Split('/');
                    }
                    else
                    {
                        sFecha = Valor.Split('-');
                    }
                }

                sRegresa = string.Format("{0}-{1}-{2}", sFecha[0], sFecha[1], sFecha[2]);
            }
            catch (Exception ex)
            {
                sRegresa = ex.Message;
                sRegresa = "";
            }

            return sRegresa;
        }

        private string GetHora(string Valor)
        {
            string sRegresa = "";
            //Valor = Fg.Right(Valor, 8);
            string[] sFecha = Valor.Split(' ');
            string[] sHora = null; 

            try
            {
                if (sFecha.Length == 2)
                {
                    sHora = sFecha[1].Split(':');
                    //sRegresa = string.Format("{0}:{1}:{2}", sFecha[0], sFecha[1], sFecha[2]);
                    sRegresa = string.Format("{0}:{1}:{2}", sHora[0], sHora[1], sHora[2]);
                }
            }
            catch (Exception ex)
            {
                sRegresa = ex.Message;
                sRegresa = "";
            }

            return sRegresa;
        }
        #endregion Funciones y Procedimientos

        #region Guardar Informacion
        public bool Guardar_001_XML__Log(ResponseRecetaElectronica Respuesta)
        {
            bool bRegresa = false;
            string sSql = "";
            ///string sFolio_SESEQ = "";


            sSql = string.Format("Exec spp_INT_SESEQ__RecetasElectronicas_0000_XML__Log " +
                " @UMedica = '{0}', @Folio_SESEQ = '{1}', @InformacionXML = '{2}', @TipoDeProceso = '{3}' ",
                sIdUMedica, sFolio_SESEQ, sInformacion_XML, (int)tpProceso);

            if (!leer.Exec(sSql))
            {
                Respuesta.Estatus = 201;
                Respuesta.Error = "Error al registrar la información en el log";
            }
            else
            {
                if (!leer.Leer())
                {
                    Respuesta.Estatus = 3;
                    Respuesta.Error = "Folio interno no asignado";
                }
                else
                {
                    sIdEmpresa = leer.Campo("IdEmpresa");
                    sIdEstado = leer.Campo("IdEstado");
                    sIdFarmacia = leer.Campo("IdFarmacia");
                    sFolioRegistro_XML = leer.Campo("Folio");

                    ////bRegresa = Guardar_002_General(Respuesta);
                    bRegresa = true; 
                }
            }

            return bRegresa;
        }

        private bool Guardar_001_XML(ResponseRecetaElectronica Respuesta)
        {
            bool bRegresa = false;
            string sSql = "";
            ///string sFolio_SESEQ = "";


            sSql = string.Format("Exec spp_INT_SESEQ__RecetasElectronicas_001_XML " +
                " @UMedica = '{0}', @Folio_SESEQ = '{1}', @InformacionXML = '{2}', @TipoDeProceso = '{3}' ",
                sIdUMedica, sFolio_SESEQ, sInformacion_XML, (int)tpProceso); 

            if (!leer.Exec(sSql))
            {
                Error.GrabarError(leer, "Guardar_001_XML"); 
                Respuesta.Estatus = 201;
                Respuesta.Error = "Error al registrar la información";
            }
            else
            {
                if (!leer.Leer())
                {
                    Respuesta.Estatus = 3;
                    Respuesta.Error = "Folio interno no asignado";
                }
                else
                {
                    sIdEmpresa = leer.Campo("IdEmpresa");
                    sIdEstado = leer.Campo("IdEstado");
                    sIdFarmacia = leer.Campo("IdFarmacia");
                    sFolioRegistro_XML = leer.Campo("Folio");

                    bRegresa = Guardar_002_General(Respuesta);
                    ////bRegresa = true; 
                }
            }

            return bRegresa; 
        }

        private bool Guardar_002_General(ResponseRecetaElectronica Respuesta)
        {
            bool bRegresa = false;
            string sSql = "";

            string sFolioReceta = "";
            string sFechaReceta = "";
            string sFechaEnvioReceta = "";

            string sFolioAfiliacionSPSS = ""; 
            string sFechaIniciaVigencia = "";
            string sFechaTerminaVigencia = "";
            string[] sVigencias = null;
            string sRangoVigencia = ""; 

            string sBeneficiarioNombre = "";
            string sBeneficiarioApPaterno = "";
            string sBeneficiarioApMaterno = "";
            string sBeneficiarioSexo = "";
            string sExpediente = "";
            string sFechaNacimiento = ""; 

            string sFolioOportunidades = "";
            string sEsPoblacionAbierta = ""; 

            string sMedicoClave = "";
            string sMedicoNombre = "";
            string sMedicoApPaterno = "";
            string sMedicoApMaterno = "";
            string sMedicoCedula = "";

            string sEsResurtible = "";
            string sNumeroDePartidas = "";
            string sProgramaAtencion = "";

            clsLeer datos = new clsLeer();
            datos.DataTableClase = leerXML.Tabla("General");
            bRegresa = datos.Registros > 0;

            while (datos.Leer())
            {
                sFolioRegistro = "*";
                sFolioReceta = datos.Campo(sNombre_CampoFolio);
                sFechaReceta = GetFecha(datos.Campo(sNombre_CampoFechaReceta)) + " " + GetHora(datos.Campo(sNombre_CampoFechaReceta));
                sFechaEnvioReceta = GetFecha(datos.Campo(sNombre_CampoFechaEnvio))+ " " + GetHora(datos.Campo(sNombre_CampoFechaEnvio));

                sFolioAfiliacionSPSS = datos.Campo("FolioDeReferencia");
                sRangoVigencia = datos.Campo("Vigencia");
                ////sVigencias = sRangoVigencia.Split('-'); 
                ////sFechaIniciaVigencia = GetFecha(sVigencias[0]);
                ////sFechaTerminaVigencia = GetFecha(sVigencias[1]);

                //sFechaIniciaVigencia = GetFecha(datos.Campo("vigenciaDerechos"));
                sFechaTerminaVigencia = GetFecha(datos.Campo("Vigencia"));
                sFechaIniciaVigencia = GetFechaIniciaVigencia(sFechaTerminaVigencia);


                sExpediente = datos.Campo("expediente");
                sBeneficiarioNombre = datos.Campo("NombreDelBeneficiario");
                sBeneficiarioApPaterno = datos.Campo("");
                sBeneficiarioApMaterno = datos.Campo("");
                sBeneficiarioSexo = datos.Campo("sexo");
                sFechaNacimiento = GetFecha(datos.Campo("fechaDeNacimiento"));

                sFolioOportunidades = datos.Campo("");
                sEsPoblacionAbierta = datos.Campo("");

                sMedicoClave = datos.Campo("idMedico");
                sMedicoNombre = datos.Campo("NombreDeQuienPrescribe");
                sMedicoApPaterno = datos.Campo("");
                sMedicoApMaterno = datos.Campo("");
                sMedicoCedula = datos.Campo("CedulaDeQuienPrescribe");

                sEsResurtible = datos.Campo("isResurtible"); ;
                sNumeroDePartidas = datos.Campo("noResurtible");
                sProgramaAtencion = datos.Campo("Programa");


                sExpediente_SESEQ = sExpediente;
                sFolioReceta_SESEQ = sFolioReceta; 

                ////sIntervencionCauses = datos.Campo("noIntervCause");
                sSql = string.Format("Exec spp_INT_SESEQ__RecetasElectronicas_0001_General \n" + 
                    "\t@IdEmpresa = '{0}', @IdEstado = '{1}', @IdFarmacia = '{2}', @Folio = '{3}', @FolioReceta = '{4}', \n" + 
                    "\t@FechaReceta = '{5}', @FechaEnvioReceta = '{6}', @FolioAfiliacionSPSS = '{7}', @FechaIniciaVigencia = '{8}', @FechaTerminaVigencia = '{9}', \n" + 
                    "\t@Expediente = '{10}', @NombreBeneficiario = '{11}', @ApPaternoBeneficiario = '{12}', @ApMaternoBeneficiario = '{13}', @Sexo = '{14}', \n" + 
                    "\t@FechaNacimientoBeneficiario = '{15}', @FolioAfiliacionOportunidades = '{16}', @EsPoblacionAbierta = '{17}', \n" + 
                    "\t@ClaveDeMedico = '{18}', @NombreMedico = '{19}', @ApPaternoMedico = '{20}', @ApMaternoMedico = '{21}', @CedulaDeMedico = '{22}', @FolioXML = '{23}', \n" + 
                    "\t@TipoDeProceso='{24}', @EsResurtible = '{25}', @NumeroDePartidas = '{26}' \n", 
                    sIdEmpresa, sIdEstado, sIdFarmacia, sFolioRegistro, 

                    sFolioReceta, sFechaReceta, sFechaEnvioReceta, 
                    sFolioAfiliacionSPSS, sFechaIniciaVigencia, sFechaTerminaVigencia,

                    sExpediente, sBeneficiarioNombre, sBeneficiarioApPaterno, sBeneficiarioApMaterno, sBeneficiarioSexo, sFechaNacimiento, 
                    sFolioOportunidades, sEsPoblacionAbierta,
                    sMedicoClave, sMedicoNombre, sMedicoApPaterno, sMedicoApMaterno, sMedicoCedula, sFolioRegistro_XML,
                    (int)tpProceso, sEsResurtible, sNumeroDePartidas, sProgramaAtencion  
                    ); 

                if (!leer.Exec(sSql))
                {
                    Error.GrabarError(leer, "Guardar_002_General"); 
                    Respuesta.Estatus = 202;
                    Respuesta.Error = "Error al registrar la información";
                    bRegresa = false;
                    break;
                }
                else
                {
                    bRegresa = leer.Leer();
                    sFolioRegistro = leer.Campo("Folio");
                    iRecepcionPrevia = leer.CampoInt("Tiene_RecepcionPrevia");

                    if (!bRegresa)
                    {
                        Respuesta.Estatus = 202;
                        Respuesta.Error = "Error al registrar la información: No se genero el folio interno";
                        break; 
                    }
                }

                if (bRegresa)
                {
                    bRegresa = Guardar_003_Causes(sFolioReceta, Respuesta);

                    if (!bRegresa)
                    {
                        break; 
                    }
                }
            }

            return bRegresa; 
        }

        private bool Guardar_003_Causes(string FolioReceta, ResponseRecetaElectronica Respuesta)
        {
            bool bRegresa = true;
            string sSql = "";
            string sIntervencionCauses = "";


            //clsLeer datos = new clsLeer();
            //datos.DataTableClase = leerXML.Tabla("Causes");
            //bRegresa = datos.Registros > 0;

            //while (datos.Leer())
            //{
            //    sIntervencionCauses = datos.Campo("noIntervCause");
            //    sSql = string.Format("Exec spp_INT_SESEQ__RecetasElectronicas_0002_Causes " +
            //        " @IdEmpresa = '{0}', @IdEstado = '{1}', @IdFarmacia = '{2}', @Folio = '{3}', @NoIntervencionCause = '{4}'  ",
            //        sIdEmpresa, sIdEstado, sIdFarmacia, sFolioRegistro, sIntervencionCauses);

            //    if (!leer.Exec(sSql))
            //    {
            //        Error.GrabarError(leer, "Guardar_003_Causes"); 
            //        Respuesta.Estatus = 203;
            //        Respuesta.Error = "Error al registrar la información";
            //        bRegresa = false;
            //        break;
            //    }
            //}


            if (bRegresa)
            {
                bRegresa = Guardar_004_Diagnosticos(FolioReceta, Respuesta);
            }

            return bRegresa;
        }

        private bool Guardar_004_Diagnosticos(string FolioReceta, ResponseRecetaElectronica Respuesta)
        {
            bool bRegresa = true;
            string sSql = "";
            string sCIE10 = "";
            string sDescripcionDiagnostico = "";


            //clsLeer datos = new clsLeer();
            //datos.DataTableClase = leerXML.Tabla("Diagnostico");
            //bRegresa = datos.Registros > 0;

            //while (datos.Leer())
            //{
            //    sCIE10 = datos.Campo("cie10");
            //    sDescripcionDiagnostico = datos.Campo("descDiagnostico");

            //    sSql = string.Format("Exec spp_INT_SESEQ__RecetasElectronicas_0003_Diagnosticos " +
            //        " @IdEmpresa = '{0}', @IdEstado = '{1}', @IdFarmacia = '{2}', @Folio = '{3}', @CIE10 = '{4}', @DescripcionDiagnostico = '{5}' ",
            //        sIdEmpresa, sIdEstado, sIdFarmacia, sFolioRegistro, sCIE10, sDescripcionDiagnostico);

            //    if (!leer.Exec(sSql))
            //    {
            //        Error.GrabarError(leer, "Guardar_004_Diagnosticos"); 
            //        Respuesta.Estatus = 204;
            //        Respuesta.Error = "Error al registrar la información";
            //        bRegresa = false;
            //        break;
            //    }
            //}


            if (bRegresa)
            {
                bRegresa = Guardar_005_Medicamentos(FolioReceta, Respuesta);
            }

            return bRegresa;
        }

        private bool Guardar_005_Medicamentos(string FolioReceta, ResponseRecetaElectronica Respuesta)
        {
            bool bRegresa = true;
            string sSql = "";
            string sClaveSSA = "";
            int iCantidad = 0;
            string sProgramaAtencion = "";
            //int iRecepcionDuplicada = 0; 


            clsLeer datos = new clsLeer();
            datos.DataRowsClase = leerXML.Tabla("Insumos").Select(string.Format(" {0} = '{1}' ", sNombre_CampoFolio, FolioReceta));
            //bRegresa = datos.Registros > 0; //// 20200217.1700 // Remover de forma temporal 


            while (datos.Leer())
            {
                sClaveSSA = datos.Campo("ClaveInsumo");
                iCantidad = datos.CampoInt("CantidadRequerida");
                sProgramaAtencion = datos.Campo("Programa");
                //iRecepcionDuplicada = datos.CampoInt("CantidadRequerida");  


                sSql = string.Format("Exec spp_INT_SESEQ__RecetasElectronicas_0004_Insumos \n" +
                    "\t@IdEmpresa = '{0}', @IdEstado = '{1}', @IdFarmacia = '{2}', @Folio = '{3}', \n" +
                    "\t@ClaveSSA = '{4}', @CantidadRequerida = '{5}', @RecepcionDuplicada = '{6}', @ClavePrograma = '{7}' \n",
                    sIdEmpresa, sIdEstado, sIdFarmacia, sFolioRegistro, sClaveSSA, iCantidad, iRecepcionPrevia, sProgramaAtencion 
                    );

                if (!leer.Exec(sSql))
                {
                    Error.GrabarError(leer, "Guardar_005_Medicamentos"); 
                    Respuesta.Estatus = 205;
                    Respuesta.Error = "Error al registrar la información";
                    bRegresa = false;
                    break; 
                }
            }

            return bRegresa;
        }
        #endregion Guardar Informacion
    }
}

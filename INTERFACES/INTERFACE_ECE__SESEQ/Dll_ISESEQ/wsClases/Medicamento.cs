...
namespace Dll_ISESEQ.wsClases
{
    public class Medicamento
    {
        ...
        TipoProcesoReceta tipoDeProceso = TipoProcesoReceta.Ninguno; 
        ...
        public string FechaDeSurtido
        {
            get { return sFechaDeSurtido; } 
            set { sFechaDeSurtido = value; }
        }

        public TipoProcesoReceta TipoDeProceso
        {
            get { return tipoDeProceso; }
            set { tipoDeProceso = value; }
        }

        public string NumeroDeReceta
        {
            get { return sNumeroDeReceta; }
            set { sNumeroDeReceta = value; }
        }
        
        public int IdImagen
        {
            get { return iIdImagen; }
            set { iIdImagen = value; }
        }

        public int TipoDeImagen
        {
            get { return iTipoDeImagen; }
            set { iTipoDeImagen = value; }
        }

        public string ImagenB64
        {
            get { return sImagenB64; }
            set { sImagenB64 = value; }
        }
        ...
        public string GetString__Digitalizacion()
        {
            string sRegresa = "";
            ...
            if (listaMedicamentos != null)
            {
                if (listaMedicamentos.Count > 0)
                {
                    ...
                    {
                        sRegresa += string.Format("{0}<{1}>\n", getTabs(iTab_01), "Digitalizacion");
                        sRegresa += string.Format("{0}<{1}>{2}</{1}>\n", getTabs(iTab_02), "TipoDePeticion", (int)itemMedicamento.TipoDeProceso);
                        sRegresa += string.Format("{0}<{1}>{2}</{1}>\n", getTabs(iTab_02), "noReceta", itemMedicamento.NumeroDeReceta);

                        sRegresa += string.Format("{0}<{1}>{2}</{1}>\n", getTabs(iTab_02), "idImagen", itemMedicamento.IdImagen);
                        sRegresa += string.Format("{0}<{1}>{2}</{1}>\n", getTabs(iTab_02), "tipoImagen", itemMedicamento.TipoDeImagen);

                        sRegresa += string.Format("{0}<{1}>{2}</{1}>\n", getTabs(iTab_02), "imagenB64", itemMedicamento.ImagenB64);
                        sRegresa += string.Format("{0}</{1}>\n", getTabs(iTab_01), "Digitalizacion");
        ...
        private string GetString__Surtido()
        {
            ...
            if (listaMedicamentos != null)
            {
                if (listaMedicamentos.Count > 0)
                {
                    foreach (Medicamento itemMedicamento in listaMedicamentos)
                    {
                        sRegresa += string.Format("{0}<{1}>\n", getTabs(iTab_01), "Medicamento");
                        sRegresa += string.Format("{0}<{1}>{2}</{1}>\n", getTabs(iTab_02), "TipoDePeticion", (int)itemMedicamento.TipoDeProceso);
                        sRegresa += string.Format("{0}<{1}>{2}</{1}>\n", getTabs(iTab_02), "noReceta", itemMedicamento.NumeroDeReceta);
                        sRegresa += string.Format("{0}<{1}>{2}</{1}>\n", getTabs(iTab_02), "fechaSurtido", itemMedicamento.sFechaDeSurtido);
                        sRegresa += string.Format("{0}<{1}>{2}</{1}>\n", getTabs(iTab_02), "clave", itemMedicamento.ClaveSSA);
                        sRegresa += string.Format("{0}<{1}>{2}</{1}>\n", getTabs(iTab_02), "surtido", itemMedicamento.CantidadSurtida.ToString("###########0"));

                        sRegresa += string.Format("{0}<{1}>{2}</{1}>\n", getTabs(iTab_02), "lote", itemMedicamento.ClaveLote);
                        sRegresa += string.Format("{0}<{1}>{2}</{1}>\n", getTabs(iTab_02), "caducidad", itemMedicamento.Caducidad);

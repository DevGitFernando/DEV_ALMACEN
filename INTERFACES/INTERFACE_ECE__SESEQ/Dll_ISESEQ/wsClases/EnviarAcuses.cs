...
namespace Dll_ISESEQ.wsClases
{
    public class EnviarInformacionAcuses
    {
        ...
        public EnviarInformacionAcuses(clsDatosConexion DatosDeConexion)
        {
            ...
            acuses = new clsLeer(); 
            Error = new clsGrabarError(datosDeConexion, GnDll_SII_SESEQ.DatosApp, "EnviarAcuses"); 
        }
        ...
        public void EnviarAcuses()
        {
            ...
            else
            {
                ...
                while (acuses.Leer())
                {
                    ...
                    sIdEstado = acuses.Campo("IdEstado");
                    sIdFarmacia = acuses.Campo("IdFarmacia");
                    sFolio_SESEQ = acuses.Campo("FolioInterface");

                    EnviarAcuses_Cancelaciones(sIdEmpresa, sIdEstado, sIdFarmacia, sFolio_SESEQ);
                }
            }
        ...
        private bool EnviarAcuses_Cancelaciones(string IdEmpresa, string IdEstado, string IdFarmacia, string Folio_SESEQ)
        {
            bool bRegresa = false;
            ResponseAcuseXML respuesta = new ResponseAcuseXML(datosDeConexion, IdEmpresa, IdEstado, IdFarmacia);
            bRegresa = respuesta.EnviarAcusesCancelacionReceta(Folio_SESEQ, true);
            return bRegresa; 
        }
    }
}

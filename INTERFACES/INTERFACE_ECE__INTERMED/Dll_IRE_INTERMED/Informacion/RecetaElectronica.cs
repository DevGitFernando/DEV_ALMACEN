...
using Dll_IRE_INTERMED.wsClases;
...
namespace Dll_IRE_INTERMED.Informacion
{
    public class RecetaElectronica
    {
        ...
        public RecetaElectronica(clsDatosConexion DatosConexion)
        {
            ...
            Error = new clsGrabarError(datosDeConexion, GnDll_SII_RE_INTERMED.DatosApp, "RecetaElectronica");
            Error.NombreLogErorres = "INT_RE_INTERMED__CtlErrores";
        ...
        {
            string sSql = string.Format("Exec spp_INT_RE_INTERMED__GetRecetas @Clues = '{0}', @TipoDocumento = '{1}'", Clues, TipoDocumento);
        ...
        public DataSet GetRecetaElectronica(string Clues, string FolioDeReceta)
        {
            string sSql = string.Format("Exec spp_INT_RE_INTERMED__GetRecetas_Especifico @Clues = '{0}', @FolioDeReceta = '{1}'", Clues, FolioDeReceta);
        ...
        private bool GuardarConfirmacion(ResponseRecetaElectronica Respuesta)
        {
            ...
            while (datos.Leer())
            {
                ...
                clsGrabarError.LogFileError("Log integración receta : " + sSql);
                if (!leer.Exec(sSql))
                {
                    Respuesta.HuboError = true;

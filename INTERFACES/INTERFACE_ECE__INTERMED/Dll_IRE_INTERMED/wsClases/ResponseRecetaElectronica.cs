...
namespace Dll_IRE_INTERMED.wsClases
{
    ...
    public class ResponseRecetaElectronica
    {
        ...
        {
            get { return bHuboError; }
            set { bHuboError = value; }
        }
        ...
        {
            DataSet dtsRegresa = new DataSet("Respuesta");
            DataTable dtTable = new DataTable("Resultados");
            int iErrores = 0;
            ...
            if (itemsRespuesta != null)
            {
                foreach (ResponseRecetaElectronicaItem item in itemsRespuesta)
                {
                    object[] obj = { item.Campo, item.Valor };
                    dtTable.Rows.Add(obj); 
                    ////sRegresa += string.Format("<{0}>{1}</{0}>", item.Campo, item.Valor);
                }
            }
            ...
            if (bHuboError)
            {
                dtTable.TableName = "Errores";
            }

            dtsRegresa.Tables.Add(dtTable.Copy());
            return dtsRegresa.Copy();
        }

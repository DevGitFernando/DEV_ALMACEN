...
namespace Dll_IRE_SIGHO.Clases
{
    public class ClsReplicacioneRecetaElectronica
    {
        ...
        public ClsReplicacioneRecetaElectronica(clsDatosConexion DatosConexion)
        {
        ...
        //            IRE_SIGHO.Url = sUrlDestino;
        //            leer.DataSetClase = IRE_SIGHO.GetRecetasElectronicas(sClues_Emisor, sTipo);
        ...
        public bool ObtenerRecetasElectronica_Especifica(string CLUES, string FolioDeReceta)
        {
            ...
            Parametros.Add("CLUES", CLUES);
            Parametros.Add("FolioDocumento", FolioDeReceta);
            ...
            try
            {
                ...
                if (leerDestinos.Leer() && bRegresa)
                {
                    ...
                    sIdFarmacia = leerDestinos.Campo("IdFarmacia");
                    sSSL = leerDestinos.CampoBool("SSL") ? "s" : "";
                    sUrlDestino = string.Format("http{0}://{1}/{2}/{3}", sSSL, leerDestinos.Campo("Servidor"), leerDestinos.Campo("WebService"), leerDestinos.Campo("PaginaWeb"));

                    System.Console.WriteLine("Clues : " + sClues_Emisor);
                    System.Console.WriteLine(sUrlDestino);

                    IRE_SIGHO.Url = sUrlDestino;
                    //leer.DataSetClase = IRE_SIGHO.GetRecetaElectronica(sClues_Emisor, FolioDeReceta);

                    string Informacion_XML = Rest.GetResponse(sUrlDestino, Parametros, Method.POST);

                    dtsInformacion.ReadXml(new XmlTextReader(new StringReader(Informacion_XML)));

                    leer.DataSetClase = leerXML.DataSetClase = dtsInformacion;
        ...
        //                if (leer.Leer())
        //                {
        //                    LeerErrores.DataSetClase = IRE_SIGHO.SendAcuseRecetasElectronicas(sClues_Emisor, leer.DataSetClase);
        ...
        //                            sSSL = string.Format("Update G Set Procesado = 1 From INT_RE_SIGHO__RecetasElectronicas_0001_General G " +
        ...
        private DataSet ObetenerRecetasElectronicasAtendidas()
        {
            ...
            string sSql = string.Format("Exec spp_INT_RE_SIGHO__RecetasElectronicas_0012_ObetenerRecetasAtendidas");
        ...
        {
            bool bRegresa = true;
            string sSql = string.Format("Select * From INT_RE_SIGHO__CFG_Farmacias_UMedicas (NoLock) Order By Referencia_SIADISSEP");

            if (CLUES != "")
            {
                sSql = string.Format("Select * From INT_RE_SIGHO__CFG_Farmacias_UMedicas (NoLock) Where Referencia_SIADISSEP = '{0}' ", CLUES);
            }
        ...
        private bool Guardar_General(DataSet Informacion)
        {
            ...
            while (datos.Leer())
            {
                ...
                sFolioReceta = datos.Campo("FolioDocumento");
                sFechaReceta = datos.Campo("FechaDocumento");
                sFechaEnvioReceta = DateTime.Now.ToString("yyyy-MM-dd");

                sFolioAfiliacionSPSS = datos.Campo("FolioReferenciaAfiliacion");
                sRangoVigencia = datos.Campo("FechaVigenciaFin");
                ...
                sFechaTerminaVigencia = datos.Campo("FechaVigenciaFin");
                sFechaIniciaVigencia = datos.Campo("FechaVigenciaInicio");
                ...
                sFechaNacimiento = datos.Campo("FechaDeNacimientoBeneficiario");
                ...
                sMedicoApPaterno = datos.Campo("ApPaternoMedico");
                sMedicoApMaterno = datos.Campo("ApMaternoMedico");
                sMedicoCedula = datos.Campo("CedulaProfesional");


                sSql = string.Format("Exec spp_INT_RE_SIGHO__RecetasElectronicas_001_XML " +
                ...
                sSql = string.Format("Exec spp_INT_RE_SIGHO__RecetasElectronicas_0001_General " +
                ...
                else
                {
                    if (leer.Leer())
                    {
                        sFolioGeneral = leer.Campo("Folio");
                    }
                }
            }
            ...
            if (bRegresa)
            {
                bRegresa = Guardar_002_Causes(sFolioReceta, Informacion);
            }

            return bRegresa;
        }

        private bool Guardar_002_Causes(string sFolioReceta, DataSet Informacion)
        {
            bool bRegresa = true;
            //string sSql = "";
            ...
            if (bRegresa)
            {
                bRegresa = Guardar_003_Diagnosticos(sFolioReceta, Informacion);
            }

            return bRegresa;
        }

        private bool Guardar_003_Diagnosticos(string sFolioReceta, DataSet Informacion)
        {
            bool bRegresa = true;
            string sSql = "";
            string sCIE10 = "";
            ...
            clsLeer datos = new clsLeer();
            datos.DataSetClase = Informacion;
            datos.DataTableClase = datos.Tabla("Diagnostico");
            //bRegresa = datos.Registros > 0;

            while (datos.Leer())
            {
                sFolio = sFolioReceta; // datos.Campo("Folio");
                sCIE10 = datos.Campo("ClaveCIE10");
                sDescripcionDiagnostico = datos.Campo("DescripcionCIE10");
            ...
            if (bRegresa)
            {
                bRegresa = Guardar_004_Medicamentos(sFolioReceta, Informacion);
            }

            return bRegresa;
        }

        private bool Guardar_004_Medicamentos(string sFolioReceta, DataSet Informacion)
        {
            bool bRegresa = true;
            string sSql = "";
            string sClaveSSA = "";
            ...
            clsLeer datos = new clsLeer();
            datos.DataSetClase = Informacion;
            datos.DataTableClase = datos.Tabla("Medicamento");
            //bRegresa = datos.Registros > 0;

...
---   5, 34, 188, 224, 313  
	...
	If exists ( Select Name From tempdb..sysobjects (noLock) Where Name like '#tmpF_XB_Surtidos%' and xType = 'U' )  
		Drop Table tempdb..#tmpF_XB_Surtidos 

	select space(2) as IdEstado, space(4) as IdFarmacia, space(20) as CodigoCliente Into #tmpF_XB_Surtidos where 1  = 0  
	Insert Into #tmpF_XB_Surtidos 
	...
	If exists ( Select Name From tempdb..sysobjects (noLock) Where Name like '#tmpPeriodos_X_Surtidos%' and xType = 'U' )  
		Drop Table tempdb..#tmpPeriodos_X_Surtidos 
	
	Select top 0 identity(int, 1, 1) as Orden, 
		cast('' as varchar(10)) as FechaInicial, cast('' as varchar(10)) as FechaFinal  
	Into #tmpPeriodos_X_Surtidos 
...
Declare 	
	...
	Begin 
		...
		Insert Into #tmpPeriodos_X_Surtidos ( FechaInicial, FechaFinal ) 
...
--		Drop Table INT_ND__Dispensacion 
	...
	Begin 
		...
		/*
			...
			If Exists ( Select * From Sysobjects (NoLock) Where Name = 'INT_ND__PRCS_Surtidos' and xType = 'U' ) 
		...
		*/ 
		...
		/*
			...
			If Exists ( Select * From Sysobjects (NoLock) Where Name = 'INT_ND__PRCS_Surtidos' and xType = 'U' ) 
				Truncate Table INT_ND__PRCS_Surtidos 
		...
		BEGIN 
			...
			FETCH NEXT FROM #cursor_002 Into @IdFarmacia, @CodigoCliente  
				...
				BEGIN 	
						...
						Exec spp_INT_ND_GenerarSurtidos 

---------------------------------------------------------------------------------------------------------------------------------- 
---------------------------------------------------------------------------------------------------------------------------------- 
If Exists ( Select Name From SysObjects(NoLock) Where Name = 'spp_INT_MA__FACT_GetInformacion_ExportarTXT_Remision' and xType = 'P' )
    Drop Proc spp_INT_MA__FACT_GetInformacion_ExportarTXT_Remision
Go--#SQL
  
Create Proc spp_INT_MA__FACT_GetInformacion_ExportarTXT_Remision
(
...
------------------------------------------- Obtener la informacion base del proceso 
	Select 
		R.IdEmpresa, R.IdEstado, R.IdFarmaciaGenera, 
	...
	Into #tmpDetalles 
	From
		--FACT_Facturas F (NoLock) -- FACT_Remisiones -- FACT_Facturas 
	--Inner Join
		FACT_Remisiones R (NoLock) 
		--On ( F.IdEmpresa = R.IdEmpresa and F.IdEstado = R.IdEstado and F.IdFarmaciaGenera = R.IdFarmaciaGenera and F.FolioRemision = R.FolioRemision )  	
	Inner Join VentasDet D (NoLock) 
		On ( R.IdEmpresa = D.IdEmpresa and R.IdEstado = D.IdEstado and R.IdFarmacia = D.IdFarmacia 
			and R.FolioRemision = D.FolioRemision and R.FolioRemision = D.FolioRemision ) 
	...
	Where R.IdEmpresa = @IdEmpresa and R.IdEstadoGenera = @IdEstado and R.IdFarmaciaGenera = @IdFarmaciaGenera and R.FolioRemision = @FolioRemision 
...
------------------------------------------- Obtener la informacion adicional 
	...
	Where EsRecetaManual = 1 and len(NumeroReceta) <= 8 
		and ( NumeroReceta not like 'G%' or NumeroReceta not like 'E%' or NumeroReceta not like 'E%' ) 
...
------------------------------------ SALIDA FINAL 
	Select	
		IdEmpresa, IdEstado, IdFarmaciaGenera, IdFarmacia, Farmacia, Proveedor_MA, D.FechaRemision, 
		D.FolioRemision, EsRecetaManual, NumeroReceta, Elegibilidad, 

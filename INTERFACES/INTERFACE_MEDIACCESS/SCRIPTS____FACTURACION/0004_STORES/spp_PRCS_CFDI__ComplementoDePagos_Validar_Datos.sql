---------------------------------------------------------------------------------------------------------------------------------------------------- 
If Exists ( Select * From Sysobjects (NoLock) Where Name = 'spp_PRCS_CFDI__ComplementoDePagos_Validar_Datos' and xType = 'P' ) 
   Drop Proc spp_PRCS_CFDI__ComplementoDePagos_Validar_Datos 
Go--#SQL 

--Exec spp_PRCS_CFDI__ComplementoDePagos_Validar_Datos   @IdEmpresa = '002', @IdEstado = '09', @IdFarmacia = '0001', @RFC = 'MAC011123M80', @Detallado = 0   

Create Proc spp_PRCS_CFDI__ComplementoDePagos_Validar_Datos   
( 
...
Set DateFormat YMD 
	...
	Exec sp_FormatearTabla CFDI_ComplementoDePagos__CargaMasiva 
...
----------------------------------- FORMATEAR VALORES 
	...
	From CFDI_ComplementoDePagos__CargaMasiva C (NoLock) 	


	
	Update C Set ExisteFactura = 1, UUID = F.UUID, ValorNominal = F.Importe, 
		NumParcialidad = IsNull(dbo.fg_CFDI_ComplementoPagos_Parcialidad ( F.IdEmpresa, F.IdEstado, F.IdFarmacia, F.Serie, F.Folio ), 0), 
		Importe_Abonos = IsNull(dbo.fg_CFDI_ComplementoPagos_Acumulado ( F.IdEmpresa, F.IdEstado, F.IdFarmacia, F.Serie, F.Folio ), 0)  
	From CFDI_ComplementoDePagos__CargaMasiva C (NoLock) 	
...
----------------------------------- FACTURAS NO EXISTENTES  
	...
	From CFDI_ComplementoDePagos__CargaMasiva P (NoLock)  

---------------------------------------------------------------------------------------------------------------------------
If Exists ( Select * From Sysobjects (NoLock) Where Name = 'spp_Mtto_CFDI_Documentos_Generados_VP' and xType = 'P' )
   Drop Proc spp_Mtto_CFDI_Documentos_Generados_VP 
Go--#SQL 

--	select * from CFDI_Documentos

Create Proc spp_Mtto_CFDI_Documentos_Generados_VP 
( 
...
Set NoCount On
	...
	If @Status = 'A' 
		Begin 
			If Not Exists 
				( 
					Select * From CFDI_Documentos_VP (NoLock) 
					Where IdEmpresa = @IdEmpresa and IdEstado = @IdEstado and IdFarmacia = @IdFarmacia 
						  and Serie = @Serie and Folio = @Folio 
				) 
			Begin 
				Insert Into CFDI_Documentos_VP ( GUID, IdEmpresa, IdEstado, IdFarmacia, FechaRegistro, 
					    ...
					    XMLFormaPago, XMLCondicionesPago, XMLMetodoPago, TipoDocumento, TipoInsumo, IdFarmaciaReferencia, IdEstadoReferencia
						) 
	...
	Else 
		Begin 
			...
			From CFDI_Documentos_VP F (NoLock) 
			Where IdEmpresa = @IdEmpresa and IdEstado = @IdEstado and IdFarmacia = @IdFarmacia 
				  and Serie = @Serie and Folio = @Folio			  
		End 
...
---------------------------------------------------------------------------------------------------------------------------
If Exists ( Select * From Sysobjects (NoLock) Where Name = 'spp_Mtto_CFDI_Documentos_Generados_Detalles_VP' and xType = 'P' )
   Drop Proc spp_Mtto_CFDI_Documentos_Generados_Detalles_VP  
Go--#SQL 

Create Proc spp_Mtto_CFDI_Documentos_Generados_Detalles_VP 
( 
	@IdEmpresa varchar(3) = '001', @IdEstado varchar(2) = '21', @IdFarmacia varchar(4) = '0001', 
...
Set NoCount On 
	...
	If Not Exists ( Select * From CFDI_Documentos_VP_Conceptos (NoLock) 
					Where IdEmpresa = @IdEmpresa and IdEstado = @IdEstado and IdFarmacia = @IdFarmacia 
						  and Serie = @Serie and Folio = @Folio and 
						  Partida = @Partida and IdProducto = @IdProducto and CodigoEAN = @CodigoEAN ) 
	Begin 
		Insert Into CFDI_Documentos_VP_Conceptos ( 
			   ...
			   TasaIva, SubTotal, Iva, Importe, TipoImpuesto ) 
		Select @GUID, @IdEmpresa, @IdEstado, @IdFarmacia, @Serie, @Folio, @Partida, 
			   @IdProducto, @CodigoEAN, @UnidadDeMedida, @Cantidad, 
			   @PrecioUnitario, @DescuentoPorc, @PrecioUnitarioFinal,
			   @TasaIva, @SubTotal, @Iva, @Total, @TipoImpuesto 
	End 
...
------------------------------------------------------------------------------------------------------------------------------------ 
If Exists ( Select Name From Sysobjects (NoLock) Where Name = 'spp_Mtto_CFDI_Documentos_MetodosDePago_VP' and xType = 'P' ) 
	Drop Proc spp_Mtto_CFDI_Documentos_MetodosDePago_VP    
Go--#SQL  

Create Proc spp_Mtto_CFDI_Documentos_MetodosDePago_VP 
(
	@IdEmpresa varchar(3) = '001', @IdEstado varchar(2) = '21', @IdFarmacia varchar(4) = '0001', 
	@Serie varchar(10) = '', @Folio int = 0, 	
...
Set DateFormat YMD 
	...
	Insert Into CFDI_Documentos_MetodosDePago_VP 
	(

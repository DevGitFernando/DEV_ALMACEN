...
{
    public partial class FrmGenerarCDFIs_ImportarDatos : FrmBaseExt
    {
        ...
        public FrmGenerarCDFIs_ImportarDatos() 
        {
            InitializeComponent();
        ...
        #region Form
        private void FrmGenerarCDFIs_ImportarDatos_Load(object sender, EventArgs e)
        {
            //list.LimpiarItems();
        ...
        private bool LeerHoja()
        {
            ...
            while(excel.Leer())
            {
                ...
                Cantidad = excel.CampoInt("Cantidad");
                PrecioUnitario = excel.CampoDouble("PrecioUnitario");
                TasaIva = excel.CampoInt("TasaIva");
                SubTotal = excel.CampoDouble("SubTotal");   //Math.Round(Cantidad * PrecioUnitario, 2);   
                Iva = excel.CampoDouble("Iva");   // Math.Round(SubTotal * (TasaIva / 100.00), 2);
                Total = excel.CampoDouble("Total");    //Math.Round(SubTotal + Iva, 2);   
            ...
            SubTotal = list.TotalizarColumnaDouble((int)Cols.SubTotal);
            Iva = list.TotalizarColumnaDouble((int)Cols.Iva);
            Total = list.TotalizarColumnaDouble((int)Cols.Total);

...
{
    public class devwsGenerarDocumento
    {
        devWebServices pax;
        ...
        public devwsGenerarDocumento(clsDatosConexion Conexion): this(Conexion, DtIFacturacion.PAC_Informacion) 
        {
        }

        public devwsGenerarDocumento(clsDatosConexion Conexion, PAC_Info Informacion_PAC)
        {
            ...
            pax = new devWebServices(sUrlTimbrado);
        ...
        public bool CancelarDocumento(string UUID, string RFC_Emisor, string ClaveMotivoCancelacion_SAT, string UUID_Relacionado )
        {
            ...
            int iRegresa = 0;
            string sRegresa = "";
            devWebServices xsa = new devWebServices(sUrlTimbrado);
            WSTimbradoCFDIService Timbrar;
        ...
        public bool GenerarDocumento(string XML_Base, string TipoDocumento)
        {
            ...
            devWebServices xsa = new devWebServices(sUrlTimbrado);
            WSTimbradoCFDIService Timbrar;
            acuseCFDI respuesta = new acuseCFDI();
            ...
            try
            {
                ...
                acc.password = sUsuarioTimbrado; //"pruebasWS";
                acc.usuario = sPasswordTimbrado; //"pruebasWS";


                Timbrar = xsa.GetReceiver();
                respuesta = Timbrar.TimbrarCFDI(acc, sXmlEnvio);
                ...
                {
                    bRegresa = true;
                    sXmlConTimbreFiscal = respuesta.xmlTimbrado; 
                    //sXmlConTimbreFiscal = sRegresa;
                }
                else 
                {
                    bRegresa = false;
                    sMensajeDeError = string.Format("{0} - {1}", respuesta.codigoError, respuesta.error);
                    bOcurrioError_AlGenerar = true;
                }
            }
            catch (Exception ex)
        ...
        public bool ConsultarCreditos(PAC_Info RFC_Emisor)
        {
            ...
            //devWebServices xsa = new vsWebServices(sUrlTimbrado);
            //IService Timbrar;

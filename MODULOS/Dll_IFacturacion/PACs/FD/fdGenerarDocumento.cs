...
namespace Dll_IFacturacion.FD
{
    ...
    public class fdGenerarDocumento
    {
        ...
        public bool CancelarDocumento( string RFC_Receptor, double Importe, string CFDI, string ClaveMotivoCancelacion_SAT, string UUID_Relacionado )
        {
            string[] listaCFDI = { CFDI };

            return CancelarDocumento(RFC_Receptor, Importe, listaCFDI, ClaveMotivoCancelacion_SAT, UUID_Relacionado);
        }

        public bool CancelarDocumento( string RFC_Receptor, double Importe, string[] ListaCFDI, string ClaveMotivoCancelacion_SAT, string UUID_Relacionado )
        {
            //ArrayOfString listaCFDI = new ArrayOfString();
            List<DetalleCFDICancelacion> listaCFDI = new List<DetalleCFDICancelacion>();
            DetalleCFDICancelacion itemCancelar;
            int iPosicion = 0;

            foreach(string sUUID in ListaCFDI)
            {
                itemCancelar = new DetalleCFDICancelacion();
                itemCancelar.RFCReceptor = RFC_Receptor;
                itemCancelar.Total = Convert.ToDecimal(Importe);
                itemCancelar.TotalSpecified = true;
                itemCancelar.UUID = sUUID;
                itemCancelar.EsCancelable = "SI";
                itemCancelar.Motivo = ClaveMotivoCancelacion_SAT;
                itemCancelar.FolioSustitucion = UUID_Relacionado;
                listaCFDI.Add(itemCancelar);
            }

            return CancelarDocumento(listaCFDI);
        }

        public bool CancelarDocumento( List<DetalleCFDICancelacion> ListaCFDI )
        {
            bool bRegresa = false;
            ...
            try
            {
                ...
                //Respuesta = cancelar.CancelarCFDIConValidacion(sUsuarioTimbrado, sPasswordTimbrado, sRFC_Emisor, ListaCFDI.ToArray(), sCertificadoPKCS12_Base64, sPasswordPKCS12);
                ...
                else
                {
                    ...
                    if(Respuesta.MensajeError.Trim() != "")
                    {
                        bRegresa = false;
                        bOcurrioError_AlGenerar = true;
                        sMensajeDeError = Respuesta.MensajeError + "\n";  
        ...
        public bool ConsultarCreditos(PAC_Info RFC_Emisor)
        {
            ...
            try
            {
                ...
                if (Respuesta.OperacionExitosa)
                {
                    ...
                    {
                        if(Paquete.FechaVencimiento >= DateTime.Now || Paquete.FechaVencimiento == null)
                        {

...
{
    public partial class FrmCFDI_NotaDeCredito : FrmBaseExt
    {
        ...
        #region Form
        private void FrmCFDI_NotaDeCredito_Load(object sender, EventArgs e)
        {
            //myGrid.LimpiarItems();
        ...
        private bool validarGenerarDocumento()
        {
            ...
            if (myGrid.GetValue(myGrid.Rows, Cols.Folio) == "" || myGrid.GetValue(myGrid.Rows, Cols.Serie) == "")
            {
                myGrid.DeleteRow(myGrid.Rows);
            }
            ...
            {
                if(myGrid.GetValueDou(i, Cols.Importe_A_Aplicar) == 0)
                {
                    bImporte = false;
                    break;
                }
            ...
            {
                bRegresa = false;
                General.msjUser("uno o mas renglones contiene cantidad a aplicar invalida, verifique.");
            }
        ...
        private bool RegistrarDocumentoElectronico() // sTipoDoctoFactura
        {
            ...
            double dPrecio_General = Math.Round(myGrid.TotalizarColumnaDou((int)Cols.Importe_Sin_Iva), 4);
            double dSubTotal_General = Math.Round(myGrid.TotalizarColumnaDou((int)Cols.Importe_Sin_Iva), 4);
            double dIVA_General = Math.Round(myGrid.TotalizarColumnaDou((int)Cols.Importe_Iva), 4);
            double dTotal_General = Math.Round(myGrid.TotalizarColumnaDou((int)Cols.Importe_A_Aplicar), 4); 
            ...
            else
            {
                ...
                for (int Renglon = 1; Renglon <= myGrid.Rows; Renglon++)
                {
                    ...
                    SubTotal = Math.Round(myGrid.GetValueDou(Renglon, (int)Cols.Importe_Sin_Iva), 4);
                    Iva = Math.Round(myGrid.GetValueDou(Renglon, (int)Cols.Importe_Iva), 4);
                    Total = myGrid.GetValueDou(Renglon, (int)Cols.Importe_A_Aplicar);
        ...
        private bool ValidarColumnas()
        {
            ...
            bRegresa = excel.ExisteTablaColumna(1, "Serie") && excel.ExisteTablaColumna(1, "Folio") && excel.ExisteTablaColumna(1, "TasaIVA") && excel.ExisteTablaColumna(1, "Importe_A_Aplicar");
        ...
        private bool CargarInformacionDeHoja()
        {
            ...
            leer.Exec("Truncate table FACT_CFDI_NotasDeCredito__CargaMasiva ");
            ...
            switch (ColActiva)
            {
                ...
                case Cols.Folio:
                    ...
                    if (sSerie != "" && sFolio != "")
                    {
                        ...
                        myGrid.SetValue(myGrid.ActiveRow, (int)Cols.Serie, "");
                        myGrid.SetValue(myGrid.ActiveRow, (int)Cols.Folio, "");
            ...
            if (!bConError)
            {
                if (bEncontrado)
                {
                    ...
                    else
                    {
                        ...
                        myGrid.SetValue(myGrid.ActiveRow, (int)Cols.Serie, sSerie);
                        myGrid.SetValue(myGrid.ActiveRow, (int)Cols.Folio, sFolio);
        ...
        private void grdNota_Advance(object sender, FarPoint.Win.Spread.AdvanceEventArgs e)
        {
            if (chkMultiples.Checked)
            {
                if ((myGrid.ActiveRow == myGrid.Rows) && e.AdvanceNext)
                {
                    if (myGrid.GetValue(myGrid.ActiveRow, Cols.Folio) != "" && myGrid.GetValueDou(myGrid.ActiveRow, Cols.Importe_A_Aplicar) != 0.0)
                    {
        ...
        private void chkMultiples_CheckedChanged(object sender, EventArgs e)
        {
            if (!chkMultiples.Checked)
            {
                if (myGrid.Rows > 1)
                {
                    if (General.msjConfirmar("Desactivar la función de multiples facturas borrará la información. \n\n Desea continuar.") == DialogResult.Yes)
                    {
                        myGrid.Limpiar(true);
                    }
                    else
        ...
        private void grdNota_KeyDown(object sender, KeyEventArgs e)
        {

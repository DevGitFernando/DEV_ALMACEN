...
namespace Dll_IFacturacion.XSA
{
    public partial class FrmFacturarRemision : FrmBaseExt
    {
        ...
        enum Cols
        {
            ...
            Ninguna = 0, 
            IdClave = 1, ClaveSSA, SAT_ClaveProductoServicio, Descripcion, PrecioUnitario, Cantidad, 
            TasaIva, SubTotal, Iva, Total, UnidadDeMedida, SAT_UnidadDeMedida  
        }
        ...
        double dF_SubTotalSinGrabar = 0;
        double dF_SubTotalGrabado = 0;
        double dF_Iva = 0;
        double dF_Total = 0;
        string sObservaciones_Facturacion = "";
        string sFolioFacturaInterna = "";

        bool bEsSustitucionDeCFDI = false; 
        ...
        public FrmFacturarRemision(string IdEmpresa, string IdEstado, string IdFarmacia, string FolioRemision, eTipoRemision TipoDeRemision,
            double F_SubTotalSinGrabar, double F_SubTotalGrabado, double F_IVA, double F_Total, string Observaciones, bool EsRemisionConcentrada):
            this(IdEmpresa, IdEstado, IdFarmacia, FolioRemision, TipoDeRemision, F_SubTotalSinGrabar, F_SubTotalGrabado, F_IVA, F_Total, Observaciones, EsRemisionConcentrada, 
                "", 0, "") 
        { 
        }
        public FrmFacturarRemision(string IdEmpresa, string IdEstado, string IdFarmacia, string FolioRemision, eTipoRemision TipoDeRemision, 
            double F_SubTotalSinGrabar, double F_SubTotalGrabado, double F_IVA, double F_Total, string Observaciones, bool EsRemisionConcentrada, 
            string Serie_Relacionada, int Folio_Relacionado, string UUID_Relacionado) 
        {
            InitializeComponent();
            CheckForIllegalCrossThreadCalls = false;

            bEsSustitucionDeCFDI = UUID_Relacionado != "";
            sSerie_Relacionada = Serie_Relacionada;
            iFolio_Relacionado = Folio_Relacionado;
            sUUID_Relacionado = UUID_Relacionado; 

            string sTitulo_01 = string.Format("{0} -- {1} ", FolioRemision, TipoDeRemision.ToString().ToUpper());
            string sTitulo_02 = string.Format("FACTURA DE {0} DE MULTIPLES REMISIONES ", TipoDeRemision.ToString().ToUpper()); 
            ...
            this.bEsRemisionConcentrada = EsRemisionConcentrada; 
            this.Text += bRemisionConcentrada ? " : " + sTitulo_01 : " : " + sTitulo_02;


            dF_SubTotalSinGrabar = F_SubTotalSinGrabar;
            dF_SubTotalGrabado = F_SubTotalGrabado;
            dF_Iva = F_IVA;
            dF_Total = F_Total;
            sObservaciones_Facturacion = Observaciones; 
            ...
            sReferencia = "RM" + sFolioRemision;

            if(bEsRemisionConcentrada)
            {
                string sValor = DtIFacturacion.QuitarSaltoDeLinea(DtIFacturacion.QuitarTabuladores(sFolioRemision));

                sReferencia = "RM" + sValor.Replace("'", "").Replace(" ", "").Replace(",", ", RM");

                //this.Text += " : " + string.Format("FACTURA DE {0} DE MULTIPLES REMISIONES ", TipoDeRemision.ToString().ToUpper());
            }

            sReferencia = DtIFacturacion.QuitarSaltoDeLinea(DtIFacturacion.QuitarTabuladores(sReferencia));

            ObtenerInformacionDeRemision();
        }
        ...
        public string FolioFacturaInterna
        {
            get { return sFolioFacturaInterna; }
        }
        ...
        private void InicializaPantalla()
        {
            ...
            formaMetodoPago = new clsFormaMetodoPago(eTipoDeFacturacion.Manual, tpVersionCFDI); 
            ...
            chkMostrarInformacionConcentrada.Checked = false;
            chkMostrarInformacionConcentrada.Enabled = false; 

            
            if  ( tpTipoDeRemision == eTipoRemision.Administracion | tpTipoDeRemision == eTipoRemision.AdministracionVentaDirecta |
            tpTipoDeRemision == eTipoRemision.IncrementoAdministracion | tpTipoDeRemision == eTipoRemision.IncrementoAdministracionVentaDirecta ) 
            {
                chkMostrarInformacionConcentrada.Enabled = true;
            }
        ...
        private bool ResetObservaciones()
        {
            ...
            if (DtIFacturacion.ReutilizarObservacionesCapturadas)
            {
                if (sObservacionesDocumento != "" || sObservacionesCondicionesDePago != "" || sObservacionesPago != "" ||
                    sObservaciones_01 != "" || sObservaciones_02 != "" || sObservaciones_03 != "" || 
                    sReferencia != "" || sReferencia_02 != "" || sReferencia_03 != "" || sReferencia_04 != "" || sReferencia_05 != ""
                    ) 
                {
                    bConfirmar = true;
                }
        ...
        private void MostrarObservaciones(bool CierreAutomatico)
        {
            FrmObservaciones f = new FrmObservaciones(dtsInformacionRemision, CierreAutomatico, bEsRemisionConcentrada);
            ...
            f.sReferencia = DtIFacturacion.QuitarTabuladores(DtIFacturacion.QuitarSaltoDeLinea(sReferencia));
            ...
            f.sIdCliente_Nombre = txtClienteNombre.Text;
        ...
        private bool validarRemision()
        {
                ...
                tpTipoDeRemision == eTipoRemision.IncrementoAdministracion | tpTipoDeRemision == eTipoRemision.IncrementoAdministracionVentaDirecta)
            {
                ...
                if (bRegresa)
                {
                    ...
                    if ( chkMostrarInformacionConcentrada.Checked ) 
                    {
        ...
        {
            iInformacionDetallada = chkMostrarInformacionConcentrada.Checked ? 0 : 1;
        ...
        private bool ObtenerInformacionDeRemision()
        {
            bool bRegresa = false;
            string sSql = "";
            int iEsRemisionConcentrada = bEsRemisionConcentrada ? 1 : 0;

            sSql = string.Format("Exec spp_FACT_CFD_GetInformacionRemision \n" +
                "\t@IdEmpresa = '{0}', @IdEstado = '{1}', @IdFarmaciaGenera = '{2}', @FolioRemision = '{3}', @EsConcentrado = '{4}' \n",
                sIdEmpresa, sIdEstado, sIdFarmacia, sFolioRemision, iEsRemisionConcentrada);
            ...
            else
            {
                if (leer.Leer())
                {
                    dtsInformacionRemision = leer.DataSetClase;
        ...
        private void ObtenerDetallesDeRemision_Insumo(int Detallado)
        {
            ...
            int iEsRemisionConcentrada = bEsRemisionConcentrada ? 1 : 0;
            int iMascara = chkAplicarMascara.Checked ? 1 : 0;
            int iMascara_Claves = chkAplicarMascara_Claves.Checked ? 1 : 0;
            int iInformacion_Lotes_EAN = chkAgregarInformacionComercial.Checked ? 1 : 0;
            string sSql = "";


            sSql = string.Format("Exec spp_FACT_CFD_GetRemision \n" +
                "\t@IdEmpresa = '{0}', @IdEstado = '{1}', @IdFarmaciaGenera = '{2}', \n\n" +
                "\t@FolioRemision = \n" +
                "\t'\n" +
                "\t{3} \n" +
                "\t', \n\n" +
                "\t@Detallado = '{4}', @Aplicar_Mascara = '{5}', @EsConcentrado = '{6}', @Anexar_Informacion_Lotes_y_Caducidades = '{7}', @Aplicar_Mascara_Claves = '{8}' \n\n",
                sIdEmpresa, sIdEstado, sIdFarmacia, sFolioRemision, Detallado, iMascara, iEsRemisionConcentrada, iInformacion_Lotes_EAN, iMascara_Claves);
            ...
            else
            {
                ...
                else
                {
                    bFacturar = true;
                    chkMostrarInformacionConcentrada.Enabled = false;
                    chkAplicarMascara.Enabled = false;
                    chkAplicarMascara_Claves.Enabled = false;
                    chkAgregarInformacionComercial.Enabled = false; 
                    ...
                    dIva = Math.Round(dTotal - dSubTotal, 2);
                } 
            }

            ////list.AnchoColumna(1, 80); 

            ////list.AnchoColumna(2, 100); 
            ////list.AnchoColumna(3, 300); 
            ////list.AnchoColumna(4, 80); 
            ////list.AnchoColumna(5, 70); 
            ////list.AnchoColumna(6, 60); 
            ////list.AnchoColumna(7, 90); 
            ////list.AnchoColumna(8, 60); 
            ////list.AnchoColumna(9, 90); 
            ////list.AnchoColumna(10, 120);


            list.AnchoColumna((int)Cols.IdClave, 80);
            list.AnchoColumna((int)Cols.ClaveSSA, 100);  
            list.AnchoColumna((int)Cols.SAT_UnidadDeMedida, 100); 
            list.AnchoColumna((int)Cols.Descripcion, 300);
            list.AnchoColumna((int)Cols.PrecioUnitario, 100); 
            list.AnchoColumna((int)Cols.Cantidad, 80);
            list.AnchoColumna((int)Cols.TasaIva, 70);
            list.AnchoColumna((int)Cols.SubTotal, 60);
            list.AnchoColumna((int)Cols.Iva, 90);
            list.AnchoColumna((int)Cols.Total, 60);
            list.AnchoColumna((int)Cols.UnidadDeMedida, 90);
            list.AnchoColumna((int)Cols.SAT_UnidadDeMedida, 120);
            ...
            lblRegistros.Text = string.Format("Registros : {0}", list.Registros.ToString("###, ###, ###, ##0"));
            lblSubTotal.Text = string.Format("{0}", dSubTotal.ToString(sFormato));
            lblIva.Text = string.Format("{0}", dIva.ToString(sFormato));
            lblTotal.Text = string.Format("{0}", dTotal.ToString(sFormato));


            lblCantidadConLetra.Text = General.LetraMoneda(dTotal);
        ...
        private bool RegistrarFacturaElectronica()
        {
            ...
            string sSAT_ClaveProductoServicio = "";
            string sSTA_UnidadDeMedida = ""; 
            ...
            string sReferenciaLocal = "";
            int iEsRelacionConRemisiones = 1; 

            ////sReferenciaLocal = DtIFacturacion.QuitarTabuladores(sReferencia);
            ////sReferenciaLocal = DtIFacturacion.QuitarTabuladores(sReferenciaLocal);
            ...
            sSql = string.Format("Exec {0} \n" +
                ...
                "\t@XMLFormaPago = '{19}', @XMLCondicionesPago = '{20}', @XMLMetodoPago = '{21}', @IdPersonalEmite = '{22}', \n" +
                "\t@TipoDocumento = '{23}', @TipoInsumo = '{24}', @IdRubroFinanciamiento = '{25}', @IdFuenteFinanciamiento = '{26}', \n" +
                "\t@UsoDeCFDI = '{27}', @Referencia_02 = '{28}', @Referencia_03 = '{29}', @Referencia_04 = '{30}', @Referencia_05 = '{31}', \n" +
                "\t@EsRelacionConRemisiones = '{32}' \n",
                sStoreEnc,  
                ...
                sXMLFormaPago, sXMLCondicionesPago, sXMLMetodoPago, DtGeneral.IdPersonal, 
                iTipoDocumento, iTipoInsumo, sRubroFinanciamiento, sFuenteFinanciamiento, DtIFacturacion.Factura_UsoCDFI, 
                sReferencia_02, sReferencia_03, sReferencia_04, sReferencia_05, iEsRelacionConRemisiones 
                );

            if (bEsSustitucionDeCFDI)
            {
                sSql += string.Format(" , @TipoRelacion = '{0}', @CFDI_Relacionado = '{1}' , @Serie_Relacionada= '{2}' , @Folio_Relacionado = '{3}' ", 
                    "04", //// Sustitución de CFDIs previos  
                    sUUID_Relacionado, sSerie_Relacionada, iFolio_Relacionado );
            }
            ...
            else
            {
                ...
                sFolioDoctoElectronico = string.Format("{0} - {1}", leerFacturar.CampoInt("Serie"), leerFacturar.CampoInt("FolioDocumento"));
                sFolioDoctoElectronico = string.Format("{0} - {1}", sSerieFactura, sFolioFactura);

                if (bEsSustitucionDeCFDI)
                {
                    sSql = string.Format("Exec spp_Mtto_FACT_ReFacturar_Remisiones \n" +
                        "\t@IdEmpresa = '{0}', @IdEstado = '{1}', @IdFarmacia = '{2}', \n" +
                        "\t@Serie = '{3}', @Folio = '{4}', @Serie_Nueva = '{5}', @Folio_Nuevo = '{6}' \n",
                        sIdEmpresa, sIdEstado, sIdFarmacia, 
                        sSerie_Relacionada, iFolio_Relacionado, 
                        cboSeries.Data, sFolioFactura
                        ); //// Marcar todas las remisiones relacionadas como CANCELADAS 
                ...
                if (bRegresa && !VistaPrevia)
                {
                    sSql = string.Format("Exec spp_Mtto_FACT_Facturar_Remisiones  " +
                        ...
                        sIdEmpresa, sIdEstado, sIdFarmacia, "*", (int)tpTipoDeRemision, sFolioRemision.Trim(),
                        sFolioDoctoElectronico, DtGeneral.IdPersonal, dF_SubTotalSinGrabar, dF_SubTotalGrabado, dF_Iva, dF_Total,
                        sObservaciones_Facturacion, 'A');
                    ...
                    if(bEsRemisionConcentrada)
                    {
                        sSql = string.Format("Exec spp_Mtto_FACT_Facturar_Remisiones  " +
                            " @IdEmpresa = '{0}', @IdEstado = '{1}', @IdFarmacia = '{2}', @FolioFactura = '{3}', @TipoDeFactura = '{4}', @FolioRemision = '{5}', " +
                            " @FolioFacturaElectronica = '{6}', @IdPersonalFactura = '{7}', @SubTotalSinGrabar = '{8}', @SubTotalGrabado = '{9}', @Iva = '{10}', @Total = '{11}', " +
                            " @Observaciones = '{12}', @Status = '{13}', @EsConcentrado = '{14}'  ",
                            sIdEmpresa, sIdEstado, sIdFarmacia, "*", (int)tpTipoDeRemision, sFolioRemision.Trim(),
                            sFolioDoctoElectronico, DtGeneral.IdPersonal, dF_SubTotalSinGrabar, dF_SubTotalGrabado, dF_Iva, dF_Total,
                            sObservaciones_Facturacion, 'A', 1);
                    }
                    ...
                    {
                        leerFacturar.Leer();
                        sFolioFacturaInterna = leerFacturar.Campo("Folio"); 
                    }
                ...
                if (bRegresa)
                {
                    for (int Renglon = 1; Renglon <= list.Registros; Renglon++)
                    {
                        ...
                        Identificador = Clave;
                        sSAT_ClaveProductoServicio = list.GetValue(Renglon, (int)Cols.SAT_ClaveProductoServicio);
                        sSTA_UnidadDeMedida = list.GetValue(Renglon, (int)Cols.SAT_UnidadDeMedida);
                        ...
                        sSql = string.Format("Exec {0} " +
                            ...
                            Identificador, Clave, Descripcion, UnidadDeMedida, Cantidad, PrecioUnitario, TasaIva,
                            SubTotal, Iva, Total, "IVA", Renglon, sSAT_ClaveProductoServicio, sSTA_UnidadDeMedida);
        ...
        private void ImprimirDocumento()
        {
            ...
            print.Imprimir(sSerieFactura, sFolioDocumento, this);
        }
        ...
        private bool ImprimirDocumento_Timbrado()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    DtIFacturacion.GenerarImpresionWebBrowser(VistaPrevia, sFileName, DtIFacturacion.GetExtras(leer.DataSetClase), sPDF, sXML, bVisualizar, null, false);
                }
            }
        ...
        {
            string sDescripcion = list.GetValue(1, (int)Cols.Descripcion);
            ...
            {
                bExisteDescripcionConcentrada = true; 
                list.SetValue(1, (int)Cols.Descripcion, f.Descripcion);
            }
        }

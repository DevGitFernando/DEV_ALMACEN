...
namespace Dll_IFacturacion.XSA
{
    public partial class FrmBancosCuentas_Emisor : FrmBaseExt
    {
        ...
        private void txtRFC_Banco_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                leer.DataSetClase = consulta.CFDI_Bancos(txtRFC_Banco.Text, "txtId_Validating");
                if (!leer.Leer())
                {
        ...
        private void txtRFC_Banco_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F1)
            {
                leer.DataSetClase = ayuda.CFDI_Bancos("txtRFC_Banco_KeyDown");
                if (leer.Leer())
                {
        ...
        private void Guardar(int iOpcion)
        {
            ...
            if (ValidaDatos())
            {
                if (cnn.Abrir())
                {
                    ...
                    sSql = string.Format("Exec spp_Mtto_FACT_CFDI_BancosCuentas__Emisor @IdEmpresa = '{0}', @IdEstado = '{1}', @IdFarmacia = '{2}', " +
        ...
        private void txtNumeroDeCuenta_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F1)
            {
                leer.DataSetClase = ayuda.CFDI_Pagos__EmisorBancos(txtRFC_Emisor.Text, txtRFC_Banco.Text, "txtNumeroDeCuenta_KeyDown");
                if (leer.Leer())
                {

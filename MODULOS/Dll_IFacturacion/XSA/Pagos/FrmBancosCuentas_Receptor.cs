...
namespace Dll_IFacturacion.XSA
{
    public partial class FrmBancosCuentas_Receptor : FrmBaseExt
    {
        ...
        private void Guardar(int iOpcion)
        {
            ...
            if (ValidaDatos())
            {
                if (cnn.Abrir())
                {
                    ...
                    sSql = string.Format("Exec spp_Mtto_FACT_CFDI_BancosCuentas__Receptor @IdEmpresa = '{0}', @IdEstado = '{1}', @IdFarmacia = '{2}', " +
        ...
        private void txtNumeroDeCuenta_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F1)
            {
                leer.DataSetClase = ayuda.CFDI_Pagos__ReceptorBancos(txtRFC_Banco.Text, "txtNumeroDeCuenta_KeyDown");
                if (leer.Leer())
                {

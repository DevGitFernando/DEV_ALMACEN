...
namespace DllFarmaciaSoft.Ventas
{
    ...
    public class clsImprimirVentas
    {
        ...
        public string RutaDestino_Archivos
        {
            get { return sRutaDestino_Archivos; }
            set { sRutaDestino_Archivos = value; }
        }

        public bool GenerarArchivos
        {
            get { return bGenerarArchivos; }
            set { bGenerarArchivos = value; }
        }
        ...
        public bool Imprimir(string Folio, string FolioFinal, double Importe, bool Desglosado)
        {
            bool bRegresa = true;

            int iDesglosado = Desglosado ? 1 : 0;

            string sSegmento, sNumeroDeSegmento, sNombreArchivo;

            string sSql = string.Format(" Exec spp_Rpt_Impresion_Ticket_Credito_Lotes_Custom_GetSegmentos @IdEmpresa = '{0}', @IdEstado = '{1}', @IdFarmacia = '{2}', @Folio = '{3}', @TipoProceso_Desglosado = {4}",
                DtGeneral.EmpresaConectada, DtGeneral.EstadoConectado, DtGeneral.FarmaciaConectada, Folio, iDesglosado);
            ...
            else
            {
                for (int i = 1; leer.Leer(); i++)
                {
                    ...
                    bRegresa = Imprimir(Folio, FolioFinal, Importe, iDesglosado, i, sSegmento, sNumeroDeSegmento, sNombreArchivo);
        ...
        private bool Imprimir(string Folio, string FolioFinal, double Importe, int Desglosado, int Iteracion, string Segmentos, string NumeroDeSegmento, string NombreArchivo)
        {
            ...
            myRpt.Add(sPrefijo + "TipoProceso_Desglosado", Desglosado);
            ...
            {
                Reporteador.MostrarInterface = true;
                bRegresa = Reporteador.GenerarReporte();
            }

            if (!bRegresa && !Reporteador.CanceladoPorUsuario)
            {

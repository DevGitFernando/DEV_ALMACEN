using System;
using QRCodeDecoder = SC_SolutionsSystem.QRCode.Codec.QRCodeDecoder;
using SC_SolutionsSystem.QRCode.Codec.Reader;
using AlignmentPatternNotFoundException = SC_SolutionsSystem.QRCode.ExceptionHandler.AlignmentPatternNotFoundException;
using InvalidVersionException = SC_SolutionsSystem.QRCode.ExceptionHandler.InvalidVersionException;
using SC_SolutionsSystem.QRCode.Geom;
using SC_SolutionsSystem.QRCode.Codec.Util;

namespace SC_SolutionsSystem.QRCode.Codec.Reader.Pattern
{
	...
	public class AlignmentPattern
	{
		...
		internal static Point[][] getCenter(bool[][] image, FinderPattern finderPattern, Point[][] logicalCenters)
		{
			...
			canvas.drawCross(centers[0][0], SC_SolutionsSystem.QRCode.Codec.Util.Color_Fields.BLUE);
			
			axis.Origin = finderPattern.getCenter(FinderPattern.UR);
			centers[sqrtCenters - 1][0] = axis.translate(- 3, 3);
			canvas.drawCross(centers[sqrtCenters - 1][0], SC_SolutionsSystem.QRCode.Codec.Util.Color_Fields.BLUE);
			
			axis.Origin = finderPattern.getCenter(FinderPattern.DL);
			centers[0][sqrtCenters - 1] = axis.translate(3, - 3);
			canvas.drawCross(centers[0][sqrtCenters - 1], SC_SolutionsSystem.QRCode.Codec.Util.Color_Fields.BLUE);
			...
			for (int y = 0; y < sqrtCenters; y++)
			{
				for (int x = 0; x < sqrtCenters; x++)
				{
					...
					if (y == 0)
					{
						...
						canvas.drawCross(centers[x][y], SC_SolutionsSystem.QRCode.Codec.Util.Color_Fields.RED);
					}
					else if (x == 0)
					{
					...
					canvas.drawCross(centers[x][y], SC_SolutionsSystem.QRCode.Codec.Util.Color_Fields.BLUE);
					canvas.drawLine(new Line(tmpPoint, centers[x][y]), SC_SolutionsSystem.QRCode.Codec.Util.Color_Fields.LIGHTBLUE);

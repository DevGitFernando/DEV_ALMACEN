If Exists ( Select Name From Sysobjects Where Name = 'spp_INF_SEND_UPD' and xType = 'P' )
	Drop Proc spp_INF_SEND_UPD 
Go--#SQL 

---  Exec spp_INF_SEND_UPD 'SII_PtoVta_EnBlanco', '[CENTRAL].[SII_OficinaCentral]', 'SII_PtoVta_EnBlanco'

-- Exec spp_INF_SEND_UPD 'SII_OficinaCentral', '[SVRPUEBLA].[SII_RegionalPuebla]', 'SII_OficinaCentral', 'CFGSC_EnvioCatalogos', 1, 1 

-- Exec spp_INF_SEND_UPD 'SII_OficinaCentral', '[SVROAXACA].[SII_Oaxaca]', 'SII_OficinaCentral', 'CFGSC_EnvioCatalogos', 1, 1 


Create Proc spp_INF_SEND_UPD 
(
...
--    delete  from #CFGC_EnvioDetalles where NombreTabla <> 'Net_CFGC_TipoCambio' 
	...
	while @@Fetch_status = 0 and @iError = 0 
		begin 
		    Set @sSql = '' 
		    -- Print @sTabla 
		    Exec spp_INF_SEND_UPD_Detalles @BaseDeDatosOrigen, @BaseDeDatosDestino, @BaseDeDatosEstructura, @sTabla, @sSql output 

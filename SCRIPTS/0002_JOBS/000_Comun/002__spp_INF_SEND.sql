-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- 
If Exists ( Select * From Sysobjects Where Name = 'spp_INF_SEND' and xType = 'P' )
...
(
	...
	@TablaDeControl varchar(500) = 'CFGSC_EnvioCatalogos',  
	@TipoEnvio int = 1, 
	@Ejecutar int = 0  		 
)
With Encryption 
...
--    delete  from #CFGC_EnvioDetalles where NombreTabla <> 'Net_CFGC_TipoCambio' 
	...
	while @@Fetch_status = 0 and @iError = 0 
		begin 
		    Set @sSql = '' 
		    -- Print @sTabla 
		    Exec spp_INF_SEND_Detalles @BaseDeDatosOrigen, @BaseDeDatosDestino, @BaseDeDatosEstructura, @sTabla, @sSql output, @iActualizado  

...
------------------------------------------ Obtener informacion 	
	Select 
		...
		(case when D.CFDI_Relacionado_CPago = '' then D.TipoRelacion else '04' end) as TipoRelacion, 
...
---		spp_FACT_CFDI_ComplementoPagos_ObtenerDatos	
	...
	----Update D Set RfcEmisorCtaOrd = '', NomBancoOrdExt = '', CtaOrdenante = '', RfcEmisorCtaBen = '', CtaBeneficiario = '' 
	----From #tmp_CFDIs_Detalles D (NoLock)  
	----Where FormaDePago in ( '01' )  

	----Update D Set NomBancoOrdExt = '', CtaOrdenante = '' 
	----From #tmp_CFDIs_Detalles D (NoLock)  
	----Where FormaDePago in ( '03' )  
...
--			Exec spp_FACT_CFDI_ComplementoPagos_ObtenerDatos  @Identificador = 958 

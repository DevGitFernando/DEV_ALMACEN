...
If Exists ( Select Name From Sysobjects (NoLock) Where Name = 'sp_Proceso_FACT_FFinanciamiento__CargaMasiva_001_Integrar_4_Insumo_Clave_Farmacia' and xType = 'P' ) 
   Drop Proc sp_Proceso_FACT_FFinanciamiento__CargaMasiva_001_Integrar_4_Insumo_Clave_Farmacia 
Go--#SQL 

Create Proc sp_Proceso_FACT_FFinanciamiento__CargaMasiva_001_Integrar_4_Insumo_Clave_Farmacia
With Encryption 
As 
Begin 
Set NoCount On 
 Set Ansi_Warnings Off  --- Especial, peligroso 
	...
	--Carga en la tabla FACT_Fuentes_De_Financiamiento_Detalles_ClavesSSA
	Insert Into FACT_Fuentes_De_Financiamiento_Detalles_ClavesSSA
		(IdFuenteFinanciamiento, IdFinanciamiento, ClaveSSA, PorcParticipacion, CantidadPresupuestadaPiezas, CantidadPresupuestada, 
		 CantidadRestante, Status, Actualizado, SAT_ClaveProducto_Servicio, SAT_UnidadDeMedida, Referencia_04, CostoBase, Porcentaje_01, Porcentaje_02)
	Select
		 Distinct IdFuenteFinanciamiento, IdFinanciamiento, ClaveSSA, PorcParticipacion, CantidadPresupuestadaPiezas, CantidadPresupuestada, 
		 CantidadRestante, Status, Actualizado, SAT_ClaveProducto_Servicio, SAT_UnidadDeMedida, Referencia_04, CostoBase, Porcentaje_01, Porcentaje_02
	From FACT_Fuentes_De_Financiamiento__CargaMasiva F (NoLock)
	Where Not Exists
	(Select * From FACT_Fuentes_De_Financiamiento_Detalles_ClavesSSA C (NoLock)
	...
	Update T
	Set 
		PorcParticipacion = F.PorcParticipacion, CantidadPresupuestadaPiezas = F.CantidadPresupuestadaPiezas, 
	...
	Insert Into FACT_Fuentes_De_Financiamiento_Detalles_ClavesSSA__Farmacia
		(IdFuenteFinanciamiento, IdFinanciamiento, IdEstado, IdFarmacia, ClaveSSA, PorcParticipacion, CantidadPresupuestadaPiezas,
	...
	Select
		F.IdFuenteFinanciamiento, F.IdFinanciamiento, F.IdEstado, F.IdFarmacia, F.ClaveSSA, F.PorcParticipacion, F.CantidadPresupuestadaPiezas,
	...
	Left Join FACT_Fuentes_De_Financiamiento_Detalles_ClavesSSA__Farmacia T (NoLock)
		On (T.IdFuenteFinanciamiento = F.IdFuenteFinanciamiento And T.IdFinanciamiento = F.IdFinanciamiento And T.IdEstado = F.IdEstado And T.IdFarmacia = F.IdFarmacia And T.ClaveSSA = F.ClaveSSA)

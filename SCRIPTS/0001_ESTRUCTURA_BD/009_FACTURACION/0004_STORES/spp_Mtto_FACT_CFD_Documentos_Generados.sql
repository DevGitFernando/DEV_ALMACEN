		...
		Begin 
			...
			Begin 
				Insert Into FACT_CFD_Documentos_Generados ( IdEmpresa, IdEstado, IdFarmacia, FechaRegistro, 
						...
						IdEstablecimiento, IdEstablecimiento_Receptor, EsRelacionConRemisiones, 
						CFDI_Relacionado, Serie_Relacionada, Folio_Relacionado 
						) 
...
Set NoCount On 
	...
	----Set @Cantidad = dbo.fg_PRCS_Redondear(@Cantidad, 2) 
	----Set @PrecioUnitario = dbo.fg_PRCS_Redondear(@PrecioUnitario, 2) 
	----Set @SubTotal = dbo.fg_PRCS_Redondear(@Cantidad * @PrecioUnitario, 2) 
	----Set @Iva = dbo.fg_PRCS_Redondear(@SubTotal * ( @TasaIva/ 100.0) , 2) 
	----Set @Total = dbo.fg_PRCS_Redondear(@SubTotal + @Iva, 2)  

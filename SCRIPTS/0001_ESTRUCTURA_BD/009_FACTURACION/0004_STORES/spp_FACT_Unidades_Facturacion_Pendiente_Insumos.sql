If Exists ( Select * From Sysobjects (NoLock) Where Name = 'spp_FACT_Unidades_Facturacion_Pendiente_Insumos' and xType = 'P' ) 
   Drop Proc spp_FACT_Unidades_Facturacion_Pendiente_Insumos  
Go--#SQL 
   
Create Proc spp_FACT_Unidades_Facturacion_Pendiente_Insumos 
( 
	...
	Where Exists 
		( 
			...
			Where V.IdEmpresa = L.IdEmpresa and V.IdEstado = L.IdEstado and V.IdFarmacia = L.IdFarmacia and L.FolioVenta = V.FolioVenta 
			      ...
			      and (case when L.ClaveLote Like '%*%' then 1 else 0 end) = 0 

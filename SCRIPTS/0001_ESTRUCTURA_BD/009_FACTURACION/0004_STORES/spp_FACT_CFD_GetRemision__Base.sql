...
( 
	...
	@IdFarmaciaGenera varchar(4) = '0001',  @FolioRemision varchar(max) = '4623', 
	@Detallado bit = 1, @Aplicar_Mascara bit = 0, @Identificador_UUID varchar(500) = '', 
	@MostrarResumen int = 0, @EsConcentrado int = 0, @Anexar_Informacion_Lotes_y_Caducidades int = 0    
) 
With Encryption		
As 
Begin 
...
------------------- INFORMACION BASE  
	...
	Select	
			...
			E.IdFarmaciaGenera, E.FolioRemision, 
	...
	-- Where D.ClaveSSA = '010.000.0106.00' 
	Group by E.IdEmpresa, E.IdEstado, E.IdFarmaciaGenera, E.FolioRemision, E.TipoDeRemision, D.ClaveSSA, E.TipoDeRemision, E.TipoInsumo 
...
------------------------------------------ Anexar Lotes y Caducidades 
	...
	Begin 
		...
		( 
			...
			Inner Join #tmpDetalles D (NoLock) On ( C.IdEmpresa = D.IdEmpresa and C.IdEstado = D.IdEstado and C.IdFarmaciaGenera = D.IdFarmaciaGenera and C.FolioRemision = D.FolioRemision )
			Where 
...
--		spp_FACT_CFD_GetRemision  
		...
		Update E Set Descripcion = E.Descripcion + space(3) + char(10) + L.Informacion 
			-- 'EAN: ' + E.CodigoEAN + '	LOTE: ' + E.ClaveLote + '	CADUCIDAD: ' + IsNull('' + convert(varchar(7), L.FechaCaducidad, 120), '')  
		From #tmpDetalles E  
		Inner Join #tmpDetalles__Lotes L (NoLock) On ( E.Keyx = L.Keyx ) 
...
----------------- Obtener datos 	
	...
	FETCH NEXT FROM #cursorDetalles Into @sDatos  
		...
		BEGIN 	
			...
			If @iIteraciones = 0 	
				Begin 
					Set @sDatosCaducidad = @sDatosCaducidad + @sDatos -- + ', ' -- + char(13) 
				End 
			Else 
				Begin 
					Set @sDatosCaducidad = @sDatosCaducidad + ', ' + char(13) + @sDatos -- + ', ' -- + char(13) 
				End 

			Set @iIteraciones = @iIteraciones + 1 

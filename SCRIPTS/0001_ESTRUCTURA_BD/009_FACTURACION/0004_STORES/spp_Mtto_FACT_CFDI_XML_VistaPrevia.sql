------------------------------------------------------------------------------------------------
If Exists ( Select * From Sysobjects (NoLock) Where Name = 'spp_Mtto_FACT_CFDI_XML_VP' and xType = 'P' ) 
	Drop Proc spp_Mtto_FACT_CFDI_XML_VP    
Go--#SQL  

Create Proc spp_Mtto_FACT_CFDI_XML_VP 
(	
...
Set DateFormat YMD
	...
	If Not Exists ( 
		Select * From FACT_CFDI_XML_VP (NoLock) 
		Where IdEmpresa = @IdEmpresa and IdEstado = @IdEstado and IdFarmacia = @IdFarmacia and Serie = @Serie and Folio = @Folio ) 
	Begin 
		Insert Into FACT_CFDI_XML_VP ( IdEmpresa, IdEstado, IdFarmacia, Serie, Folio, IdPAC, uf_CadenaOriginal, uf_SelloDigital, uf_CFDFolio, uf_IVenta, uf_CFDI_Info, uf_Tipo, uf_CanceladoSAT, uf_CadenaOriginalSAT, uf_SelloDigitalSAT, uf_FolioSAT, uf_NoCertificadoSAT, uf_FechaHoraCerSAT, uf_CBB, uf_ackCancelacion_SAT, uf_Xml_Base, uf_Xml_Timbrado, uf_Xml_Impresion, uf_Pdf ) 
		Select @IdEmpresa, @IdEstado, @IdFarmacia, @Serie, @Folio, @IdPAC, @uf_CadenaOriginal, @uf_SelloDigital, @uf_CFDFolio, @uf_IVenta, @uf_CFDI_Info, @uf_Tipo, @uf_CanceladoSAT, @uf_CadenaOriginalSAT, @uf_SelloDigitalSAT, @uf_FolioSAT, @uf_NoCertificadoSAT, @uf_FechaHoraCerSAT, @uf_CBB, @uf_ackCancelacion_SAT, @uf_Xml_Base, @uf_Xml_Timbrado, @uf_Xml_Impresion, @uf_Pdf
	End 	

	Select Keyx as FolioDocumento 
	From FACT_CFDI_XML_VP (NoLock) 
	Where IdEmpresa = @IdEmpresa and IdEstado = @IdEstado and IdFarmacia = @IdFarmacia and Serie = @Serie and Folio = @Folio 
 	
		
---		sp_listacolumnas  FACT_CFDI_XML  
	
---		sp_listacolumnas__Stores spp_Mtto_FACT_CFDI_XML , 1  	

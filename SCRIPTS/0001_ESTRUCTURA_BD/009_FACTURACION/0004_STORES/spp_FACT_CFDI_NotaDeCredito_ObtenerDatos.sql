------------------------------------------------------------------------------------------------------------------------------------------------------ 
If Exists ( Select * From Sysobjects (NoLock) Where Name = 'spp_FACT_CFDI_NotaDeCredito_ObtenerDatos' and xType = 'P' ) 
   Drop Proc spp_FACT_CFDI_NotaDeCredito_ObtenerDatos 
Go--#SQL  

Create Proc spp_FACT_CFDI_NotaDeCredito_ObtenerDatos  
(
	@Identificador bigint = 4  
...
------------------------------------------ Obtener informacion 	
	Select 
		...
		'E' as EfectoComprobante, 'EGRESO' as EfectoComprobante_Descripcion, 
...
---		spp_FACT_CFDI_NotaDeCredito_ObtenerDatos	
	...
	Select 
		...
		cast(0 as numeric(14,4)) as Impuestos_TasaOCuota__RetencionIVA, 			
	...
	Select 
		sum(Impuestos_Base) as Impuestos_Base, Impuesto_ImpuestoClave, Impuesto_Impuesto, Impuestos_TipoFactor, Impuestos_TasaOCuota, 
		Impuesto1 as TasaIva, sum(Impuestos_Importe) as Importe    
	Into #tmpTraslados 
	From #tmpConceptos  	

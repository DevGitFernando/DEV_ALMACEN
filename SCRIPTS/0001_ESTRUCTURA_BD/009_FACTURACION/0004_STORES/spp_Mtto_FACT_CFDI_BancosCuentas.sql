----------------------------------------------------------------------------------------------------------------- 
If Exists ( Select * From sysobjects (NoLock) Where Name = 'spp_Mtto_FACT_CFDI_BancosCuentas__Emisor' and xType = 'P' ) 
   Drop Proc spp_Mtto_FACT_CFDI_BancosCuentas__Emisor
Go--#SQL    

Create Proc spp_Mtto_FACT_CFDI_BancosCuentas__Emisor
(
	...
	If @iOpcion = 1 
		Begin 
			If Not Exists ( Select *
							From FACT_CFDI_BancosCuentas__Emisor (NoLock)
				...
				Begin 
					...
					Insert Into FACT_CFDI_BancosCuentas__Emisor ( IdEmpresa, IdEstado, IdFarmacia, RFC_Emisor, RFC_Banco, NumeroDeCuenta, Status )
...
----------------------------------------------------------------------------------------------------------------- 
If Exists ( Select * From sysobjects (NoLock) Where Name = 'spp_Mtto_FACT_CFDI_BancosCuentas__Receptor' and xType = 'P' ) 
   Drop Proc spp_Mtto_FACT_CFDI_BancosCuentas__Receptor
Go--#SQL    

Create Proc spp_Mtto_FACT_CFDI_BancosCuentas__Receptor
(
...
Set NoCount On 
	...
	If @iOpcion = 1 
		Begin 
			If Not Exists ( Select *
							From FACT_CFDI_BancosCuentas__Receptor (NoLock)
							Where IdEmpresa = @IdEmpresa And IdEstado = @IdEstado And IdFarmacia = @IdFarmacia And
								  RFC_Banco = @RFC_Banco And NumeroDeCuenta = @NumeroDeCuenta) 
				Begin 
					Insert Into FACT_CFDI_BancosCuentas__Receptor ( IdEmpresa, IdEstado, IdFarmacia, RFC_Banco, NumeroDeCuenta, Status )

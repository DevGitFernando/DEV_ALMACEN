------------------------------------------------------------------------------------------------------------------------------------  
If Exists ( Select * From Sysobjects (NoLock) Where Name = 'spp_FACT_Rpt_ReporteDeOperacion_11____2N_Farmacias' and xType = 'P' ) 
   Drop Proc spp_FACT_Rpt_ReporteDeOperacion_11____2N_Farmacias 
Go--#SQL   

--	Exec spp_FACT_Rpt_ReporteDeOperacion_11____2N_Farmacias @AplicarFiltroFolios = 1, @FolioInicial = 19469, @FolioFinal = 19469  

Create Proc	 spp_FACT_Rpt_ReporteDeOperacion_11____2N_Farmacias  
(
...
--		End
	...
	Select
		...
		--cast('' As varchar(200)) As NumReceta, 
	Into #Salida
	From vw_FACT_Remisiones_Detalles S (NoLock)
	Inner Join FACT_Remisiones____RPT F On (S.IdEmpresa = F.IdEmpresa And S.IdEstado = F.IdEstado And S.IdFarmacia = F.IdFarmaciaGenera And S.FolioRemision = F.FolioRemision)
...
--		Exec spp_FACT_Rpt_ReporteDeOperacion_11____2N_Farmacias @AplicarFiltroFolios = 1, @FolioInicial = 19469, @FolioFinal = 19469  
	...
	Update S Set --S.IdCliente = F.IdCliente, S.IdSubCliente = F.IdSubCliente, 
		S.FechaVenta = Convert(varchar(10), F.FechaRegistro, 120)
	From #Salida S (NoLock) 

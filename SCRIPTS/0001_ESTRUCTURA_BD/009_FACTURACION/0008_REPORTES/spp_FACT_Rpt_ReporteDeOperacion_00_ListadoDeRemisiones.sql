------------------------------------------------------------------------------------------------------------------------------------  
If Exists ( Select * From Sysobjects (NoLock) Where Name = 'spp_FACT_Rpt_ReporteDeOperacion_00_ListadoDeRemisiones' and xType = 'P' ) 
   Drop Proc spp_FACT_Rpt_ReporteDeOperacion_00_ListadoDeRemisiones 
Go--#SQL   

--	Exec spp_FACT_Rpt_ReporteDeOperacion_00_ListadoDeRemisiones @AplicarFiltroFolios = 1, @FolioInicial = 19469, @FolioFinal = 19469  

Create Proc	 spp_FACT_Rpt_ReporteDeOperacion_00_ListadoDeRemisiones  
(
...
--Año, Mes
	...
	select 
		...
		R.GUID, 
		R.FolioRemision, 
		D.IdFarmacia as  IdUnidad, cast('' as varchar(500)) as Unidad, 
		R.IdFuenteFinanciamiento, R.IdFinanciamiento, R.Financiamiento, 
		--FolioFacturaElectronica, 
		R.EsRelacionDocumento, R.FolioRelacionDocumento, R.ReferenciaDocumento, 
		R.EsRelacionFacturaPrevia, R.Serie, R.Folio, 
		R.TipoDeRemisionDesc, R.OrigenInsumoDesc, R.TipoDeInsumoDesc, 
		count(distinct R.FolioRemision) as FoliosRemisiones, 

		sum(D.Cantidad_Agrupada) as Cantidad_Agrupada, sum(Cantidad) as Cantidad, 

		'SubTotal Remisioando' = sum(D.SubTotalSinGrabar + D.SubTotalGrabado), 
		'IVA Remisionado' = sum(D.IVA), 
		'Importe Remisionado' = sum(D.Importe), 
		'SubTotal Facturable' = cast( (case when R.EsFacturable = 1 then sum(D.SubTotalSinGrabar + D.SubTotalGrabado) else 0 end) as numeric(14,4)),  
		'IVA Facturable' = cast( (case when R.EsFacturable = 1 then sum(D.IVA) else 0 end) as numeric(14,4)),  
		'Importe Facturable '= cast( (case when R.EsFacturable = 1 then sum(D.Importe) else 0 end) as numeric(14,4))  
	Into #Salida_Final  
	from #vw_FACT_Remisiones R (NoLock)  
	Inner Join FACT_Remisiones_Detalles D (NoLock) On ( R.FolioRemision = D.FolioRemision ) 
	--where GUID in ( 'c0092ec8-cebd-48ee-a940-85b5ab7dd22e', '5e107927-8b98-48df-a67d-18ff88b913f5' ) 
	--where EsRelacionDocumento = 1 
	--where GUID = '9ba3abe8-db1d-4c00-b428-511a43572101' 
	--where GUID = 'c0092ec8-cebd-48ee-a940-85b5ab7dd22e' 
	Group by 
		...
		R.EsRelacionDocumento, R.FolioRelacionDocumento, R.ReferenciaDocumento, 
		R.EsRelacionFacturaPrevia, R.Serie, R.Folio, 
		R.TipoDeRemisionDesc, R.OrigenInsumoDesc, R.TipoDeInsumoDesc, R.EsFacturable 
	Order by   
		R.FolioRemision, R.GUID  


	Update R Set Unidad = F.Farmacia 
	From #Salida_Final R 
	Inner Join vw_Farmacias F (NoLock) On ( F.IdEstado = @IdEstado and R.IdUnidad = F.IdFarmacia ) 

	--				spp_FACT_Rpt_ReporteDeOperacion_00_ListadoDeRemisiones  
	...
	Begin 
		...
		Set @sSql_Segmento = 'Insert Into #tmpResultados ( NombreTabla, EsGeneral ) select ' + char(39) + 'Remisiones' + right('0000' + cast(@iSegmento as varchar(10)), 4) + char(39) + ', 1 ' 
		Exec( @sSql_Segmento ) 

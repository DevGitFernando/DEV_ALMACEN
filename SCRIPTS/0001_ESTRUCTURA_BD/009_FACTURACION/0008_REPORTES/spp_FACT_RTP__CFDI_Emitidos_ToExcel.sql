------------------------------------------------------------------------------------------------------------------------------------------------------------------- 
If Exists ( Select * From Sysobjects (NoLock) Where Name = 'spp_FACT_RTP__CFDI_Emitidos_ToExcel' and xType = 'P' ) 
   Drop Proc spp_FACT_RTP__CFDI_Emitidos_ToExcel 
Go--#SQL 

Create Proc spp_FACT_RTP__CFDI_Emitidos_ToExcel 
( 
	@IdEmpresa varchar(3) = '001', @IdEstado varchar(2) = '13', @IdFarmacia varchar(4) = '', 
	@EsReporteGeneral int = 0, 

	@Año int = 2019, @Mes int = 0  
)  
As 
Begin 
--Set NoCount On 
Declare 
	...
	----------------------- ARMAR CRITERIOS  
	Set @sFiltro = 'Where IdEmpresa = ' + char(39) + @IdEmpresa + char(39) + char(13) 

If Exists ( Select Name From Sysobjects Where Name = 'spp_CFG_ObtenerDatos__Checador' and xType = 'P' )
	Drop Proc spp_CFG_ObtenerDatos__Checador
Go--#SQL  

Create Proc spp_CFG_ObtenerDatos__Checador 
( 
	@Tabla varchar(50) = 'Checador_Personal', @DiasDeRevision int = 2, @ParteUpDate int = 1   
...
Declare 
	...
	Set @FechaInicial = dateadd(dd, -1 * @DiasDeRevision, getdate()) 
	Set @FechaFinal = getdate() 

	Set @Criterio = 
		'Where convert(varchar(10), FechaRegistro, 120) ' + 
		' between ' + char(39) + convert(varchar(10), @FechaInicial, 120) + char(39) + ' and ' + char(39) + convert(varchar(10), @FechaFinal, 120) + char(39)  
	------print @Criterio 
	------print '' 
	------print '' 
	------print '' 	
...
------ Agregar la tabla al query, agregar nombre a la columna de salida 
	Exec(	@QueryFinal + ' As Resultado, ' + 
	    ...
	    ' From ' + @Tabla + ' (NoLock) ' + @Criterio + ' ' ) 	

---- Jesus Diaz 2K120111.1800 
	--Print @IdEstado 
	print @QueryFinal + char(13) + ' From ' + @Tabla + ' (NoLock) ' + char(13) + @Criterio + ' ' 

...
Create Procedure spp_COM_SeleccionProveedorProductos ( @IdClaveSSA varchar(4), @GUID varchar(50), @FolioPedido varchar(6) )
As
Begin
	Declare
		@sSql varchar(8000),
	...
	Select top 0 space(300) As 'GUID', Cast( '' as varchar(6) ) As FolioPedido, 
	...
	cast(0 as Numeric(14,4)) as PrecioMin, cast(0 as Numeric(14,4)) as PrecioMax, space(100) As ObservacionesPrecios,
		0 As EsSobrePrecio, 0 as ProveedorSancionado 
	Into #tmpCriterios  
	...
	Select Distinct @GUID As GUID, @FolioPedido As FolioPedido, C.IdClaveSSA_Sal, C.ClaveSSA, C.DescripcionSal, C.CodigoEAN, C.Descripcion, L.IdProveedor, 
			P.Nombre, L.PrecioUnitario, 0 as Cant_A_Pedir, cast(0 as Numeric(14,4)) as PrecioMin, 
	cast(0 as Numeric(14,4)) as PrecioMax, space(100) As ObservacionesPrecios, 0 As EsSobrePrecio, 0 as ProveedorSancionado  
	...
	Set @sSql = 
	'Insert Into Com_Pedidos_Compras ' +
	'Select ' + Char(39) + @GUID + Char(39) + ', ' + Char(39) + @FolioPedido + Char(39) + ' As FolioPedido, IdClaveSSA, ClaveSSA, DescripcionClave, CodigoEAN, Producto, IdProveedor, NombreProveedor, Precio, Cant_A_Pedir, ' +
	' PrecioMin, PrecioMax, ObservacionesPrecios, EsSobrePrecio ' +
	'From ' + @sTabla + '(NoLock) ' + 
	'Where Not Exists ( Select * From Com_Pedidos_Compras ' + 
	...
	Set @sSql = 
	'Insert Into ' + @sTabla + ' ' +
	'Select ' + Char(39) + @GUID + Char(39) + ', FolioPedido, IdClaveSSA, ClaveSSA, DescripcionClave, CodigoEAN, Producto, IdProveedor, NombreProveedor, Precio, Cant_A_Pedir,  ' +
	'PrecioMin, PrecioMax, ObservacionesPrecios, EsSobrePrecio, 0 as ProveedorSancionado ' +
	...
	Set @PrecioMax = ( Select Max(Precio) From #tmpCriterios(NoLock) )
		...
		-- Se buscan los Proveedores Sancionados
	Update #tmpCriterios Set ProveedorSancionado = 1 
	Where IdProveedor In ( Select IdProveedor From COM_OCEN_Proveedores_Sancionados(NoLock) )

	Select	T.IdProveedor, T.NombreProveedor, T.CodigoEAN, T.Producto, 
			P.Presentacion, P.ContenidoPaquete, T.Precio, T.Cant_A_Pedir, 
			0.0000 As Importe, @PrecioMin As PrecioMin, @PrecioMax As PrecioMax, T.ProveedorSancionado,  

...
Begin
	...
	*/
	...
	If @FechaFinVigencia <= (CONVERT(varchar(10), GETDATE(), 120))
		Begin 
			Set @FechaFinVigencia = (CONVERT(varchar(10), (GETDATE() + 365), 120))
	End 
	...
	if @iOpcion = 1 
	  Begin
		...
		If Not Exists ( Select * From COM_OCEN_ListaDePrecios (NoLock) 
						Where IdProveedor = @IdProveedor And CodigoEAN = @CodigoEAN )
		  Begin

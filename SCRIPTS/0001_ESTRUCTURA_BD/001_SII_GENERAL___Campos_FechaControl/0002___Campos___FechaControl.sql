...
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                ...
                Where So.Name = 'Net_Usuarios' and Sc.Name = 'FechaControl' ) 
   Alter Table Net_Usuarios Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
...
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'FarmaciaProductos_CodigoEAN_Lotes__Historico' and Sc.Name = 'FechaControl' ) 
   Alter Table FarmaciaProductos_CodigoEAN_Lotes__Historico Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
...
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'DevolucionTransferenciasEnc' and Sc.Name = 'FechaControl' ) 
   Alter Table DevolucionTransferenciasEnc Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
 
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'DevolucionTransferenciasDet' and Sc.Name = 'FechaControl' ) 
   Alter Table DevolucionTransferenciasDet Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
 
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'DevolucionTransferenciasDet_Lotes' and Sc.Name = 'FechaControl' ) 
   Alter Table DevolucionTransferenciasDet_Lotes Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
 
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'DevolucionTransferenciasDet_Lotes_Ubicaciones' and Sc.Name = 'FechaControl' ) 
   Alter Table DevolucionTransferenciasDet_Lotes_Ubicaciones Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
...
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'TransferenciasEstadisticaClavesDispensadas' and Sc.Name = 'FechaControl' ) 
   Alter Table TransferenciasEstadisticaClavesDispensadas Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
 
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'TransferenciasEnvioEnc' and Sc.Name = 'FechaControl' ) 
   Alter Table TransferenciasEnvioEnc Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
 
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'TransferenciasEnvioDet' and Sc.Name = 'FechaControl' ) 
   Alter Table TransferenciasEnvioDet Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
 
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'TransferenciasEnvioDet_Lotes' and Sc.Name = 'FechaControl' ) 
   Alter Table TransferenciasEnvioDet_Lotes Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
 
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'TransferenciasEnvioDet_LotesRegistrar' and Sc.Name = 'FechaControl' ) 
   Alter Table TransferenciasEnvioDet_LotesRegistrar Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
...
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'CatPersonalCEDIS' and Sc.Name = 'FechaControl' ) 
   Alter Table CatPersonalCEDIS Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
...
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'Pedidos_Cedis_Enc_Surtido' and Sc.Name = 'FechaControl' ) 
   Alter Table Pedidos_Cedis_Enc_Surtido Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
 
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'Pedidos_Cedis_Enc_Surtido_Atenciones' and Sc.Name = 'FechaControl' ) 
   Alter Table Pedidos_Cedis_Enc_Surtido_Atenciones Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
...
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'Pedidos_Cedis_Det_Surtido_Distribucion' and Sc.Name = 'FechaControl' ) 
   Alter Table Pedidos_Cedis_Det_Surtido_Distribucion Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
 
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'Pedidos_Cedis_Det_Surtido_Distribucion_Reproceso' and Sc.Name = 'FechaControl' ) 
   Alter Table Pedidos_Cedis_Det_Surtido_Distribucion_Reproceso Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 
...
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'Adt_OrdenesDeComprasEnc' and Sc.Name = 'FechaControl' ) 
   Alter Table Adt_OrdenesDeComprasEnc Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 

--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
If Not Exists ( Select So.Name, Sc.Name From Sysobjects So (NoLock) 
                Inner Join Syscolumns sc (NoLock) On ( So.Id = Sc.Id ) 
                Where So.Name = 'Transferencias_UUID_Registrar' and Sc.Name = 'FechaControl' ) 
   Alter Table Transferencias_UUID_Registrar Add FechaControl DateTime Not Null Default getdate() 
Go--#SQL 

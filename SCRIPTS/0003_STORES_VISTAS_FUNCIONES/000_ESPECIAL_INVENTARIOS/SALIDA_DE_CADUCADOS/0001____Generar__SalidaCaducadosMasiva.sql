...
--	Select * From VentasEnc
		...
		If Exists ( Select * from sysobjects (nolock) Where Name = 'PRCS_SMC__001__FarmaciaProductos_CodigoEAN_Lotes' and xType = 'u' ) Drop Table PRCS_SMC__001__FarmaciaProductos_CodigoEAN_Lotes

		If Exists ( Select * from sysobjects (nolock) Where Name = 'PRCS_SMC__002__FarmaciaProductos_CodigoEAN_Lotes_Ubicaciones' and xType = 'u' )  Drop Table PRCS_SMC__002__FarmaciaProductos_CodigoEAN_Lotes_Ubicaciones

		If Exists ( Select * from sysobjects (nolock) Where Name = 'PRCS_SMC__003__FarmaciaProductos_CodigoEAN' and xType = 'u' )  Drop Table PRCS_SMC__003__FarmaciaProductos_CodigoEAN

		If Exists ( Select * from sysobjects (nolock) Where Name = 'PRCS_SMC__004__FarmaciaProductos' and xType = 'u' )  Drop Table PRCS_SMC__004__FarmaciaProductos
...
Declare @FolioVenta varchar(32) = '*', @FolioMovto VarChar(32) = '*', @FechaSistema varchar(10) = '', @IdCaja varchar(2) = '', @IdPersonal varchar(6) = '0001',
		...
		Set @IdEstado = '11' 
		Set @IdFarmacia = '3005' 
		Set @sObservaciones = 'SALIDA MASIVA DE CADUCADOS'  
		Set @EsConsignacion = 0 


		Set @sObservaciones = 'SALIDA MASIVA DE CADUCADOS (CONSIGNACION)'  
		Set @EsConsignacion = 1 
		...
		Select 
			datediff(month, getdate(), FechaCaducidad) as MesesCaducidad, 
		...
		Into PRCS_SMC__001__FarmaciaProductos_CodigoEAN_Lotes 
		From FarmaciaProductos_CodigoEAN_Lotes (NoLock) 
		Where IdEmpresa = @IdEmpresa and IdEstado = @IdEstado And IdFarmacia = @IdFarmacia 
			and ExistenciaEnTransito <= 0 
			and ExistenciaSurtidos <= 0 
			and Existencia > 0 
			and datediff(month, getdate(), FechaCaducidad) <= 0  
			and EsConsignacion = @EsConsignacion  


		Update L Set TasaIVA = P.TasaIva
		From PRCS_SMC__001__FarmaciaProductos_CodigoEAN_Lotes L 
		...
		Select 
			...
			cast(0 as numeric(14,4)) as IVA, 
			cast(0 as numeric(14,4)) as Total 
		Into PRCS_SMC__002__FarmaciaProductos_CodigoEAN_Lotes_Ubicaciones 
		...
		Where IdEmpresa = @IdEmpresa and IdEstado = @IdEstado And IdFarmacia = @IdFarmacia 
			...
			(
				Select * 
				From PRCS_SMC__001__FarmaciaProductos_CodigoEAN_Lotes L (NoLock) 
		...
		From PRCS_SMC__002__FarmaciaProductos_CodigoEAN_Lotes_Ubicaciones U (NoLock) 
		Inner join PRCS_SMC__001__FarmaciaProductos_CodigoEAN_Lotes L (NoLock) 
			On ( L.IdEmpresa = U.IdEmpresa and L.IdEstado = U.IdEstado and L.IdFarmacia = U.IdFarmacia and L.IdSubFarmacia = U.IdSubFarmacia 
			and L.IdProducto = U.IdProducto and L.CodigoEAN = U.CodigoEAN and L.ClaveLote = U.ClaveLote and L.SKU = U.SKU ) 

		Update L Set SubTotal = Existencia * Costo 
		From PRCS_SMC__002__FarmaciaProductos_CodigoEAN_Lotes_Ubicaciones L 
		...
		Select 
			...
			sum(IVA) as IVA, 
			sum(Total) as Total 
		Into PRCS_SMC__003__FarmaciaProductos_CodigoEAN 
		From PRCS_SMC__001__FarmaciaProductos_CodigoEAN_Lotes 
		Group by 
		...
		Select 
			...
			sum(IVA) as IVA, 
			sum(Total) as Total 			
		Into PRCS_SMC__004__FarmaciaProductos 
		From PRCS_SMC__001__FarmaciaProductos_CodigoEAN_Lotes 
		Group by 
			IdEmpresa, IdEstado, IdFarmacia, IdProducto, 
			TasaIVA, Costo, 
...
--	sp_listacolumnas__stores spp_Mtto_VentasInformacionAdicional , 1 
	...
	From PRCS_SMC__004__FarmaciaProductos 
...
----	sp_listacolumnas		MovtosInv_Det_CodigosEAN_Lotes_Ubicaciones 
	...
	From PRCS_SMC__003__FarmaciaProductos_CodigoEAN 
	Group by 
	...
	From PRCS_SMC__002__FarmaciaProductos_CodigoEAN_Lotes_Ubicaciones 

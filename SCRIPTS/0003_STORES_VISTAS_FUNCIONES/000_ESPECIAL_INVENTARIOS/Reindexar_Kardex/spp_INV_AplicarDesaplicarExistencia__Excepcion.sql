...
Create Proc spp_INV_AplicarDesaplicarExistencia ( @IdEmpresa varchar(3) = '001', @IdEstado varchar(2) = '25', @IdFarmacia varchar(4) = '0001', 
	...
	If @AfectarCostos = 1 
	   Begin 
			...
			Update F Set UltimoCosto = A.CostoNuevo, 
				 CostoPromedio = (case when (A.Existencia + A.Cantidad) <= 0 Then F.CostoPromedio 
				 Else Round( ((A.Existencia * A.CostoAnterior) + (A.Cantidad * A.CostoNuevo)) / (A.Existencia + A.Cantidad), 4 ) end )   
			From FarmaciaProductos F (NoLock) 
			Inner Join #tmpArticulos A (NoLock) 
				On ( F.IdEmpresa = A.IdEmpresa and F.IdEstado = A.IdEstado and F.IdFarmacia = A.IdFarmacia and F.IdProducto = A.IdProducto ) 	        

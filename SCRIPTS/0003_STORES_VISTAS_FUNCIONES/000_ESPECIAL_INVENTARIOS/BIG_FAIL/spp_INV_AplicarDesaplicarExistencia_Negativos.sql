...
Alter Proc spp_INV_AplicarDesaplicarExistencia ( @IdEmpresa varchar(3) = '001', @IdEstado varchar(2) = '25', @IdFarmacia varchar(4) = '0001', 
...
--  spp_INV_AplicarDesaplicarExistencia
	...
	Inner Join FarmaciaProductos_CodigoEAN_Lotes F (NoLock) 
		On ( M.IdEmpresa = F.IdEmpresa and M.IdEstado = F.IdEstado and M.IdFarmacia = F.IdFarmacia and M.IdProducto = F.IdProducto and M.CodigoEAN = F.CodigoEAN and M.ClaveLote = F.ClaveLote ) 			
	Where M.IdEmpresa = @IdEmpresa and M.IdEstado = @IdEstado and M.IdFarmacia = @IdFarmacia and M.FolioMovtoInv = @FolioMovto 		
---------------- Obtener los movimientos de inventario 	
...
----		On ( F.IdEmpresa = A.IdEmpresa and F.IdEstado = A.IdEstado and F.IdFarmacia = A.IdFarmacia and F.IdProducto = A.IdProducto and 
----			 F.CodigoEAN = A.CodigoEAN and F.ClaveLote = A.ClaveLote ) 
----	Where ( F.Existencia + A.Cantidad ) < 0 			 
-------------------- Actualizar Existencia Negativos 

---------------- Actualizar Existencia 
	Update F Set Existencia = ( F.Existencia + A.Cantidad )
	From FarmaciaProductos F (NoLock) 
	Inner Join #tmpArticulos A (NoLock) 
		On ( F.IdEmpresa = A.IdEmpresa and F.IdEstado = A.IdEstado and F.IdFarmacia = A.IdFarmacia and F.IdProducto = A.IdProducto ) 
...
---------------- Actualizar Existencia en tablas de Inventarios 
	Update E Set Existencia = F.Existencia 
	...
	Inner Join FarmaciaProductos_CodigoEAN_Lotes F (NoLock) 
		On ( F.IdEmpresa = A.IdEmpresa and F.IdEstado = A.IdEstado and F.IdFarmacia = A.IdFarmacia and F.IdProducto = A.IdProducto and 
			 F.CodigoEAN = A.CodigoEAN and A.ClaveLote = F.ClaveLote ) 	
---------------- Actualizar Existencia en tablas de Inventarios 


---------------- Marcar Movimiento en tablas de Inventarios 
	Update M Set MovtoAplicado = 'S'
	...
	If @AfectarCostos = 1 
	   Begin 
			...
			Update F Set UltimoCosto = A.CostoNuevo, 
				 CostoPromedio = Round( ((A.Existencia * A.CostoAnterior) + (A.Cantidad * A.CostoNuevo)) / (A.Existencia + A.Cantidad), 4 )    
			From FarmaciaProductos F (NoLock) 
			Inner Join #tmpArticulos A (NoLock) 
				On ( F.IdEmpresa = A.IdEmpresa and F.IdEstado = A.IdEstado and F.IdFarmacia = A.IdFarmacia and F.IdProducto = A.IdProducto ) 	        

If Exists ( Select Name From Sysobjects (NoLock) Where Name = 'sp_GetPermisos_UsuariosDetallesOrdenar' and xType = 'P' )
   Drop proc sp_GetPermisos_UsuariosDetallesOrdenar 
Go--#SQL  

Create Proc sp_GetPermisos_UsuariosDetallesOrdenar ( @sArbol varchar(4) = 'COM', @iRamaPadre int, @iNivel int = 1 output ) 
with Encryption 
As 
Begin 
	Set DateFormat YMD 
...
declare @iRama int 
    ...
    WHILE ( @@FETCH_STATUS = 0 ) 
        BEGIN
            ...
            If @@NESTLEVEL < 32
            Begin
			   Insert Into tmpGetPermisosFinal ( Arbol, Rama, Nombre, Padre, FormaLoad, GrupoOpciones, IdOrden, TipoRama, RutaCompleta ) 
               ...
               Set @iNivel = @iNivel + 1
               Exec sp_GetPermisos_UsuariosDetallesOrdenar @sArbol, @iRama, @iNivel out 
               Set @iNivel = @iNivel - 1
            End

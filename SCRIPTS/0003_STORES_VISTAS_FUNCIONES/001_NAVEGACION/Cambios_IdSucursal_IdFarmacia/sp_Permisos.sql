...
Declare @iRaiz int, 
	...
	Inner Join CatPersonal PR (NoLock) On ( G.IdEstado = PR.IdEstado and G.IdFarmacia = PR.IdFarmacia and PR.Status = 'A' )  
	Inner Join Net_Privilegios_grupo P (NoLock) On ( G.IdEstado = P.IdEstado and G.IdFarmacia = P.IdFarmacia and G.IdGrupo = P.IdGrupo and P.Status = 'A' ) 
	Inner Join Net_Navegacion N (NoLock) On ( P.Arbol = N.Arbol and P.RutaCompleta = N.RutaCompleta and N.Status = 'A' ) 
	Where G.IdEstado = @sIdEstado and G.IdFarmacia = @sIdEntidad and N.Arbol = @sArbol and  M.LoginUser = @sUserLogin 

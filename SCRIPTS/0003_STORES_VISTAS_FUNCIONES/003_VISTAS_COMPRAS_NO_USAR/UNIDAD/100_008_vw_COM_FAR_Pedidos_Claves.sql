----------------------------------------------------------------- 
If Exists ( Select Name From Sysobjects (NoLock) Where Name = 'vw_COM_FAR_Pedidos_Claves' and xType = 'V' ) 
	Drop View vw_COM_FAR_Pedidos_Claves 
Go--#SQL 	

Create View vw_COM_FAR_Pedidos_Claves 
As 
	Select M.IdEmpresa, M.Empresa, M.IdEstado, M.Estado, M.ClaveRenapo, M.IdFarmacia, M.Farmacia, 
		...
		S.ContenidoPaquete, S.Presentacion, D.CantidadPiezas as Cantidad, IsNull(Cantidad_Surtir, 0 ) as CantidadSurtir, 
		D.Cantidad_Pedido as CantidadCajas, 
		M.Observaciones, M.StatusPedido, M.Status 
	From vw_COM_FAR_PedidosEnc M (NoLock) 
	Inner Join COM_FAR_Pedidos_Claves D (NoLock) 

...
If Exists ( Select Name From SysObjects(NoLock) Where Name = 'spp_Rpt_Vales_Por_Mes' And xType = 'P' )
	Drop Proc spp_Rpt_Vales_Por_Mes
Go--#SQL

Create Procedure spp_Rpt_Vales_Por_Mes 
( 
	@IdEmpresa varchar(3) = '001', @IdEstado varchar(2) = '21', 
	@IdFarmacia varchar(4) = '1188', @iAño int = 2012, @iMes int = 10,  @iMostrarResultado smallint = 1 
)
With Encryption
As
Begin 
	...
	------------------------------------------------------------------------------------------------------------------------
    If @iMostrarResultado = 0 
      Begin 
		Select	IdEmpresa, IdEstado, IdFarmacia, FolioVale as 'Folio Vale', FolioVenta as 'Folio Venta', NumReceta as 'Numero Receta', Cantidad,  
				IdBeneficiario, Beneficiario, Poliza, FechaRegistro as 'Fecha Registro', FechaCanje as 'Fecha Canje',
				IdCliente, Cliente, IdSubCliente, SubCliente, IdPrograma, Programa, IdSubPrograma, SubPrograma, Status  
		From #tmpEmitidos(NoLock) 
		Order By IdFarmacia, FolioVale 

		Select	IdEmpresa, IdEstado, IdFarmacia, Folio, FolioVale as 'Folio Vale', 
	...
	If @iMostrarResultado = 1 
      Begin 
		Select *, @iAño as Año, @sNombreMes as Mes From #tmpEmitidos(NoLock) Order By IdFarmacia, FolioVale 
	  End 

	If @iMostrarResultado = 2 
      Begin  
		Select *, @iAño as Año, @sNombreMes as Mes From #tmpRegistrados(NoLock) Order By IdFarmacia, FolioVale 
	  End 

	If @iMostrarResultado = 3 
      Begin  
		Select * From #tmpRegistrados_Detallado(NoLock) Order By IdFarmacia, FolioVale, IdProducto, CodigoEAN 
	  End 

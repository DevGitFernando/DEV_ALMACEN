If Exists ( Select Name From Sysobjects (NoLock) Where Name = 'sp_Proceso_IntegrarInventarioInicial_004___Almacen' and xType = 'P' ) 
   Drop Proc sp_Proceso_IntegrarInventarioInicial_004___Almacen 
Go--#SQL 

Create Proc sp_Proceso_IntegrarInventarioInicial_004___Almacen  
( 
...
--------------------- Existencia capturada 
	...
	IsNull((
		Select sum(Cantidad) From #tmpBase B 
		Where B.IdSubFarmacia = F.IdSubFarmacia and B.IdProducto = F.IdProducto and B.CodigoEAN = F.CodigoEAN and B.ClaveLote = F.ClaveLote 
			and B.IdPasillo = F.IdPasillo and B.IdEstante = F.IdEstante and B.IdEntrepaño = F.IdEntrepaño 
	), 0)  
	From #tmpUbicaciones F 	
--------------------- Existencia capturada 	

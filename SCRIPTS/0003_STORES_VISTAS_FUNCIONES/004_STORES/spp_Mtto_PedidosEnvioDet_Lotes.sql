...
--	Select * From PedidosEnvioDet_Lotes (Nolock)

If Exists (Select Name From SysObjects(NoLock) Where Name = 'spp_Mtto_PedidosEnvioDet_Lotes' and xType = 'P')
    Drop Proc spp_Mtto_PedidosEnvioDet_Lotes
Go--#SQL
  
Create Proc spp_Mtto_PedidosEnvioDet_Lotes 
( 
	...
	*/
	...
	If @iOpcion = 1 
       Begin 
		   ...
		   If Not Exists ( Select * From PedidosEnvioDet_Lotes (NoLock) 
			  ...
			  Begin 
				Insert Into PedidosEnvioDet_Lotes ( IdEmpresa, IdEstado, IdFarmacia, IdSubFarmacia, FolioPedido, IdProducto, CodigoEAN, 
...
--			  Begin 
--			     
--				Update PedidosEnvioDet_Lotes Set 
...
--              End 
    ...
    Else 
       Begin 
			Set @sStatus = 'C'
			Update PedidosEnvioDet_Lotes Set Status = @sStatus, Actualizado = @iActualizado 
			Where IdEmpresa = @IdEmpresa And IdEstado = @IdEstado And IdFarmacia = @IdFarmacia And IdSubFarmacia = @IdSubFarmacia 

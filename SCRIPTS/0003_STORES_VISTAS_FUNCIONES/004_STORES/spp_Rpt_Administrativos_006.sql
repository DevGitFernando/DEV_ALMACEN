-----------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------- 
If Exists ( Select * From sysobjects (nolock) Where Name = 'spp_Rpt_Administrativos_006' and xType = 'P' ) 
   Drop Proc spp_Rpt_Administrativos_006 
Go--#SQL 

--		Exec spp_Rpt_Administrativos	
 
Create Proc spp_Rpt_Administrativos_006 
( 
	@TipoDispensacion tinyint = 0, 
	@FechaInicial varchar(10) = '2010-01-01', @FechaFinal varchar(10) = '2010-06-30', 
	@TipoInsumoMedicamento tinyint = 0, @MostrarPrecios tinyint = 0  
)  
With Encryption 
As 
Begin 
...
--- Obtener el Procentaje para los Productos No Licicitados/Conveniados 
	Select Top 1 @sIdEstado = IdEstado From RptAdmonDispensacion_NoSurtido (NoLock) 
...
----------------------------------  
	...
	From RptAdmonDispensacion_NoSurtido B (NoLock) 
	...
	Update B Set IdClaveSSA_Sal = I.IdClaveSSA_Sal, ClaveSSA = I.ClaveSSA, DescripcionSal = I.DescripcionSal, 
		 ...
		 TipoInsumo = (case when TipoDeClave = '01' then 2 else 1 end)  
	From RptAdmonDispensacion_NoSurtido B (NoLock) 
...
------	         Delete from RptAdmonDispensacion_NoSurtido Where EsSeguroPopular = 2 
------	         
------	      if @TipoInsumoMedicamento = 2 
------	         Delete from RptAdmonDispensacion_NoSurtido Where EsSeguroPopular = 1 	         
------	   End 
...
---------------- Datos de Beneficiario 
	...
	--Where Exists ( Select IdEstado, IdFarmacia From RptAdmonDispensacion_NoSurtido T (NoLock) 
...
--	  select top 10 * from RptAdmonDispensacion_NoSurtido (nolock) where IdGrupoPrecios = 1 
	
		
----
--- Asignar Precio 
	Update B Set 
		PrecioLicitacion = IsNull(PC.Precio, 0), 
		PrecioLicitacionUnitario = IsNull(PC.PrecioUnitario, 0), 	
		ContenidoPaquete_ClaveSSA = IsNull(PC.ContenidoPaquete, 1), 	
		PrecioLicitacion__Negado = IsNull(PC.Precio, 0), 
...
-- between convert(varchar(10), @FechaInicial, 120) and convert(varchar(10), @FechaFinal, 120)   ---   drop table RptAdmonDispensacion_NoSurtido 

------  Marcar los Precios 		
	Update B Set FechaInicial = cast(@FechaInicial as datetime),  FechaFinal = cast(@FechaFinal as datetime), 
		 ImporteEAN__Negado = (Cantidad__Negado * PrecioLicitacion__Negado), 
	     IdGrupoPrecios	= (case when PrecioLicitacion__Negado <= 0 Then 1 Else 2 End), 
		 DescripcionGrupoPrecios = (case when PrecioLicitacion__Negado <= 0 Then 'SIN PRECIO DE LICITACION' Else 'PRECIO LICITACION ASIGNADO' End)
	From RptAdmonDispensacion_NoSurtido B (NoLock) 		
...
------------------- Informacion adicional para Validación y Facturacion 
	Update B Set SubTotalLicitacion_0__Negado = ImporteEAN__Negado 
	From RptAdmonDispensacion_NoSurtido B 
	Where TipoInsumo = 1 
	
	Update B Set SubTotalLicitacion__Negado = ImporteEAN__Negado 
	From RptAdmonDispensacion_NoSurtido B 
	Where TipoInsumo = 2 	

--	select * From RptAdmonDispensacion_NoSurtido B 	
	
	Update B Set IvaLicitacion__Negado = SubTotalLicitacion__Negado * ( (TasaIva / 100.00) )
	From RptAdmonDispensacion_NoSurtido B 
	Where TipoInsumo = 2 	
	
	Update B Set TotalLicitacion__Negado = SubTotalLicitacion_0__Negado +  ( SubTotalLicitacion__Negado + IvaLicitacion__Negado )
	From RptAdmonDispensacion_NoSurtido B 	
...
--- Reporte Ambos asegurar q solo Venta muestre precios 		
	Update B Set 
		...
		PrecioLicitacion__Negado = 0, ImporteEAN__Negado = 0,  
		SubTotalLicitacion_0__Negado = 0 , SubTotalLicitacion__Negado = 0, IvaLicitacion__Negado = 0, TotalLicitacion__Negado = 0  		 
	From RptAdmonDispensacion_NoSurtido B 		
	Where EsConsignacion = 1 
------------------- Informacion adicional para Validación y Facturacion 
--------------------------------------------------------------------------------------------------------- 

--			spp_Rpt_Administrativos_006		
...
------------------------------- Quitar los precios 
	...
	Begin 
		Update R Set 
			...
			SubTotalLicitacion_0 = 0, SubTotalLicitacion = 0, IvaLicitacion = 0, TotalLicitacion = 0, 
			PrecioLicitacion__Negado = 0, ImporteEAN__Negado = 0, SubTotalLicitacion_0__Negado = 0, SubTotalLicitacion__Negado = 0, 
			IvaLicitacion__Negado = 0, TotalLicitacion__Negado = 0  		
		From RptAdmonDispensacion_NoSurtido R 
	End 
------------------------------- Quitar los precios 

...
If Exists ( Select Name From SysObjects(NoLock) Where Name = 'spp_Mtto_Vtas_RegistroServicioDomicilio' and xType = 'P' )
    Drop Proc spp_Mtto_Vtas_RegistroServicioDomicilio
Go--#SQL
  
Create Proc spp_Mtto_Vtas_RegistroServicioDomicilio 
(	
...
--- Armar folio de inventario 
    ...
    Where IdEmpresa = @IdEmpresa And IdEstado = @IdEstado and IdFarmacia = @IdFarmacia and IdTipoMovto_Inv = @TipoMovto  
...
--- Jesus Diaz 2K111217.1056 
	...
	Begin 
	    ...
	    Where IdEmpresa = @IdEmpresa and IdEstado = @IdEstado and IdFarmacia = @IdFarmacia 
			...
			( 
				Select FolioVale 
				From Vales_Servicio_A_Domicilio V 
				Where IdEmpresa = @IdEmpresa and IdEstado = @IdEstado and IdFarmacia = @IdFarmacia 
				and FolioServicioDomicilio = @Folio_Vale_Registro  	     			
			) 
...
---	select * 	from #tmpVentas_Inf 
	...
	From Vales_Servicio_A_DomicilioDet V (NoLock) 
	Inner Join vw_Productos_CodigoEAN P (NoLock) On ( V.IdProducto = P.IdProducto and V.CodigoEAN = P.CodigoEAN )
	Where V.IdEmpresa = @IdEmpresa and V.IdEstado = @IdEstado and V.IdFarmacia = @IdFarmacia and V.FolioServicioDomicilio = @Folio_Vale_Registro 	
	...
	From Vales_Servicio_A_DomicilioDet_Lotes V (NoLock) 
	Where V.IdEmpresa = @IdEmpresa and V.IdEstado = @IdEstado and V.IdFarmacia = @IdFarmacia and V.FolioServicioDomicilio = @Folio_Vale_Registro 	
-------------------------------------------------- Generar Ventas 
...
--- 	
	Select @FolioVentaNuevo as FolioNuevo, @FolioMovtoInv as FolioInventario 

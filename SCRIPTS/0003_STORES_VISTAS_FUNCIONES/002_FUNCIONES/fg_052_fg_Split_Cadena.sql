-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- 
If Exists ( select * From Sysobjects (NoLock) Where Name = 'fg_SplitCadena' and xType = 'TF' ) 
   Drop Function fg_SplitCadena 
Go--#SQL 

/* 
	select * from dbo.fg_SplitCadena('0001*2') 

	select * from dbo.fg_SplitCadena('0001') 

	select * from dbo.fg_SplitCadena('0001|0002|0003') 
*/ 

Create Function fg_SplitCadena 
(    
...
Declare 
	...
	Begin
		...
		Set @IdProducto = @Segmento -- Substring(@Segmento, @StartIndex, @EndIndex_02 - 1)
		--Set @Cantidad = replace(@Segmento, @IdProducto + @Character_02, '')  -- Substring(@Segmento, @EndIndex_02 + 1, len(@Segmento) - (@EndIndex_02 + 1))		
		...
		Insert Into @Output( Valor ) -- , Cantidad)

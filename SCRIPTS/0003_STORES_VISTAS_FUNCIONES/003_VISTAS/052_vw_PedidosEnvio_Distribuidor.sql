If Exists ( Select Name From Sysobjects (NoLock) Where Name = 'vw_PedidosEnvioEnc' and xType = 'V' ) 
	Drop View vw_PedidosEnvioEnc 
Go--#SQL 
 	
Create View vw_PedidosEnvioEnc 
With Encryption 
As 
...
------------------------------------------------------------------------------   
If Exists ( Select Name From Sysobjects (NoLock) Where Name = 'vw_PedidosEnvioDet_CodigosEAN' and xType = 'V' ) 
	Drop View vw_PedidosEnvioDet_CodigosEAN 
Go--#SQL  	

Create View vw_PedidosEnvioDet_CodigosEAN 
With Encryption 
As 
	Select M.IdEmpresa, M.Empresa, M.IdEstado, M.Estado, M.ClaveRenapo, M.IdFarmacia, M.Farmacia, M.Folio, 
		...
		1 as UnidadDeSalida, 
		D.SubTotal as Importe, D.ImpteIva as Iva, D.Importe as Total    		
	From vw_PedidosEnvioEnc M (NoLock) 
	Inner Join PedidosEnvioDet D (NoLock) On ( M.IdEmpresa = D.IdEmpresa and M.IdEstado = D.IdEstado and M.IdFarmacia = D.IdFarmacia and M.Folio = D.FolioPedido ) 
	Inner Join CatProductos P (NoLock) On ( D.IdProducto = P.IdProducto ) 
	-- Where D.Status = 'A' 
Go--#SQL
 	
------------------------------------------------------------------------------    
If Exists ( Select Name From Sysobjects (NoLock) Where Name = 'vw_PedidosEnvioDet_CodigosEAN_Lotes' and xType = 'V' ) 
	Drop View vw_PedidosEnvioDet_CodigosEAN_Lotes 
Go--#SQL  	

Create View vw_PedidosEnvioDet_CodigosEAN_Lotes  
With Encryption 
As 	
	...
	Select E.IdEmpresa, E.Empresa, E.IdEstado, E.Estado, E.ClaveRenapo, E.IdFarmacia, E.Farmacia, 
		...
		cast(D.Cant_Devuelta as int) as Cant_Devuelta, 
		cast(D.CantidadRecibida as int) as CantidadRecibida   
	From vw_PedidosEnvioDet_CodigosEAN E (NoLock) 
	Inner Join PedidosEnvioDet_Lotes D (noLock) 
	...
	Inner Join PedidosEnvioDet_Lotes_Registrar L (NoLock) 
		On ( D.IdEmpresa = L.IdEmpresa and D.IdEstado = L.IdEstado and D.IdFarmacia = L.IdFarmacia 
			 and D.IdSubFarmacia = L.IdSubFarmacia and E.Folio = L.FolioPedido and 

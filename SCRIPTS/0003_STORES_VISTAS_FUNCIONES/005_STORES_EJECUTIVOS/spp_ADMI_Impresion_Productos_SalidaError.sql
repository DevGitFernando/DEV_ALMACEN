If Exists ( Select Name From SysObjects(NoLock) Where Name = 'spp_ADMI_Impresion_Productos_SalidaError' And xType = 'P' )
	Drop Proc spp_ADMI_Impresion_Productos_SalidaError
Go--#SQL 

Create Procedure spp_ADMI_Impresion_Productos_SalidaError ( @IdEmpresa varchar(3), @IdEstado varchar(2), @IdFarmacia varchar(4) )
With Encryption
As
Begin
	...
	(
		 ...
		 CantidadSE numeric(14, 4) Not Null Default 0,
		 CostoSE numeric(14, 4) Not Null Default 0,
		 ImporteSE numeric(14, 4) Not Null Default 0,
	...
	---------------------------------------------------
	Insert Into #tmpClaves ( IdProducto, CodigoEAN, ClaveLote, CantidadSE, CostoSE, ImporteSE, IdEmpresa, IdEstado, IdFarmacia, Folio )
	Select D.IdProducto, D.CodigoEAN, D.ClaveLote, D.Cantidad as CantidadSE, D.Costo, D.Importe, D.IdEmpresa, D.IdEstado, D.IdFarmacia, E.FolioMovtoInv
	...
	Where E.IdEmpresa = @IdEmpresa And E.IdEstado = @IdEstado And E.IdFarmacia = @IdFarmacia And Left( E.FolioMovtoInv, 2 ) = 'SE' 
	...
	-------------------------------------------------------	
	If Exists ( Select Name From SysObjects(NoLock) Where Name = 'tmpADMI_Productos_SalidaError' And xType = 'U' )
	  Begin
		Drop Table tmpADMI_Productos_SalidaError
	  End	
	
	Select *
	Into tmpADMI_Productos_SalidaError

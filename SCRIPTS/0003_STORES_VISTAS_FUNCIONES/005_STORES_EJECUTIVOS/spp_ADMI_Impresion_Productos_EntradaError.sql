If Exists ( Select Name From SysObjects(NoLock) Where Name = 'spp_ADMI_Impresion_Productos_EntradaError' And xType = 'P' )
	Drop Proc spp_ADMI_Impresion_Productos_EntradaError
Go--#SQL 

Create Procedure spp_ADMI_Impresion_Productos_EntradaError ( @IdEmpresa varchar(3), @IdEstado varchar(2), @IdFarmacia varchar(4) )
With Encryption
As
Begin
	...
	(
		 ...
		 CantidadEE numeric(14, 4) Not Null Default 0,
		 CostoEE numeric(14, 4) Not Null Default 0,
		 ImporteEE numeric(14, 4) Not Null Default 0,
	...
	---------------------------------------------------
	Insert Into #tmpClaves ( IdProducto, CodigoEAN, ClaveLote, CantidadEE, CostoEE, ImporteEE, IdEmpresa, IdEstado, IdFarmacia, Folio )
	...
	Where E.IdEmpresa = @IdEmpresa And E.IdEstado = @IdEstado And E.IdFarmacia = @IdFarmacia And Left( E.FolioMovtoInv, 2 ) = 'EE' 
		And E.MovtoAplicado = 'S' And E.TipoES = 'E' --And ClaveLote Like '%*%' 
	...
	-------------------------------------------------------	
	If Exists ( Select Name From SysObjects(NoLock) Where Name = 'tmpADMI_Productos_EntradaError' And xType = 'U' )
	  Begin
		Drop Table tmpADMI_Productos_EntradaError
	  End	
	
	Select *
	Into tmpADMI_Productos_EntradaError
	...
	Select * From tmpADMI_Productos_EntradaError(NoLock) Order By IdFarmacia, DescripcionSal

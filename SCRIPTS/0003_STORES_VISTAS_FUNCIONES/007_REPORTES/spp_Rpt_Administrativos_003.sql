If Exists ( select * from sysobjects (nolock) Where Name = 'RptAdmonDispensacion_Documentos' and xType = 'U' ) 
	Drop table RptAdmonDispensacion_Documentos  
Go--#SQL  
...
-----------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------- 
If Exists ( Select * From sysobjects (nolock) Where Name = 'spp_Rpt_Administrativos_003' and xType = 'P' ) 
   Drop Proc spp_Rpt_Administrativos_003 
Go--#SQL 

----	spp_Rpt_Administrativos_003 ''

Create Proc spp_Rpt_Administrativos_003 
(   
...
--- Armar criterios para la consulta 
	...
	select V.IdEmpresa, space(500) as Empresa, 
		...
		C.IdClaveSSA_Sal, C.ClaveSSA, space(2) as IdProducto, space(2) as CodigoEAN, 0 as Renglon, L.Cantidad as Cantidad, 
		0 as PrecioUnitario, 0 as TasaIva, 0 as ImpteIva, 0 as ImporteEAN, 
		space(2) as ClaveLote, L.Cantidad as CantidadLote, 
		0 as ImpteIvaLote, 
		0 as ImporteLote, 
		0 as TipoInsumo, 
	...
	Inner Join #vw_ClavesSSA_Sales C (NoLock) On ( C.IdClaveSSA_Sal = L.IdClaveSSA_Sal ) 
	...
	@sWhere +  	' order by V.IdEmpresa, V.IdEstado, V.IdFarmacia, V.FolioVenta' 
 	--Print 	@sSql 	  
 	Exec(@sSql) 
...
---------------------------------------------------------------- 
	...
	Select * 
	Into RptAdmonDispensacion_Documentos 
	From 
	( 

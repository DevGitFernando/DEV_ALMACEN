...
If Exists( Select Name From SysObjects(NoLock) Where Name = 'spp_Rpt_Consulta_Ventas_Farmacia' And xType = 'P' ) 
	Drop Proc spp_Rpt_Consulta_Ventas_Farmacia
Go--#SQL

Create Procedure spp_Rpt_Consulta_Ventas_Farmacia ( @IdEstado varchar(2), @IdFarmacia varchar(4), @FechaInicial varchar(10), @FechaFinal varchar(10), @EsDeConsignacion varchar(1) )
With Encryption
As
Begin
	/*	@EsDeConsignacion 
	...
	*/
	...
	Select	V.IdEstado, V.IdFarmacia, Cast( '' as varchar(50) ) as Farmacia, V.IdCliente, V.IdSubCliente, L.CantidadVendida as Cantidad, 
			P.IdClaveSSA_Sal, L.ClaveLote, Cast( 0 as Numeric(14,4) ) as Precio, Cast( 0 as Numeric(14,4) ) as Importe, 
			Convert( varchar(10), V.FechaSistema, 120 ) as FechaSistema
	Into #tmpCodigos
	...
	Where V.IdEstado = @IdEstado And V.IdFarmacia = @IdFarmacia And V.TipoDeVenta = '2' 
	And Convert( varchar(10), V.FechaSistema, 120 ) Between @FechaInicial And @FechaFinal 
	...
	-- Se obtiene el nombre de las Farmacias
	Update V
	Set Farmacia = C.NombreFarmacia
	From #tmpCodigos V(NoLock)
	Inner Join CatFarmacias C(Nolock) On ( V.IdEstado = C.IdEstado And V.IdFarmacia = C.IdFarmacia )

	-- Se insertan los datos en la tabla temporal de las ventas
	Select FechaSistema, Sum( Importe ) as Total
	Into #tmpVentas
	...
	Select * From #tmpVentas(NoLock) Order By FechaSistema
End
Go--#SQL

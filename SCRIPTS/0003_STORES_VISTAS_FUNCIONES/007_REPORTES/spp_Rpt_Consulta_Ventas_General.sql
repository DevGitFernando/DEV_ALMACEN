...
If Exists( Select Name From SysObjects(NoLock) Where Name = 'spp_Rpt_Consulta_Ventas_General' And xType = 'P' ) 
	Drop Proc spp_Rpt_Consulta_Ventas_General
Go--#SQL

Create Procedure spp_Rpt_Consulta_Ventas_General ( @FechaInicial varchar(10), @FechaFinal varchar(10), @EsDeConsignacion varchar(1) )
With Encryption
As
Begin
	/*	@EsDeConsignacion 
	...
	*/
	...
	Select	V.IdEstado, Cast( '' as varchar(50) ) as Estado, V.IdCliente, V.IdSubCliente, L.CantidadVendida as Cantidad, 
	...
	-- Se obtiene el nombre de los Estados
	Update V
	Set Estado = C.Nombre
	From #tmpCodigos V(NoLock)
	Inner Join CatEstados C(Nolock) On ( V.IdEstado = C.IdEstado )

	-- Se insertan los datos en la tabla temporal de las ventas
	Select IdEstado, Estado, Sum( Importe ) as Total
	Into #tmpVentas
	...
	Select * From #tmpVentas(NoLock) Order By Estado
End
Go--#SQL

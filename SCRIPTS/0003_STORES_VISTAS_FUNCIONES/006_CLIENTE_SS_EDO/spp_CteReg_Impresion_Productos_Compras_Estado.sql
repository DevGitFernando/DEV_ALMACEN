...
If Exists ( Select Name From SysObjects(NoLock) Where Name = 'spp_CteReg_Impresion_Productos_Compras_Estado' And xType = 'P' )
	Drop Proc spp_CteReg_Impresion_Productos_Compras_Estado
Go--#SQL

Create Procedure spp_CteReg_Impresion_Productos_Compras_Estado ( @IdEstado varchar(2) = '25', @IdFarmacia varchar(4) = '',
...
Begin
	...
	---------------------------------------------------
	Insert Into #tmpClaves ( IdProducto, CodigoEAN, ClaveLote, Cantidad, Costo, Importe, IdEmpresa, IdEstado, IdFarmacia, Folio, ReferenciaDocto, IdProveedor, FechaCompra, FechaInicial, FechaFinal )
	Select	L.IdProducto, L.CodigoEAN, L.ClaveLote, IsNull( L.CantidadRecibida, 0 ) as CantidadComprada, D.CostoUnitario, D.Importe, 
			E.IdEmpresa, E.IdEstado, E.IdFarmacia, E.FolioCompra, E.ReferenciaDocto, E.IdProveedor, E.FechaRegistro, @FechaInicial, @FechaFinal
	...
	Inner Join #tmpFarmaciasProcesar F(NoLock) On ( D.IdEstado = F.IdEstado And D.IdFarmacia = F.IdFarmacia ) 
	Where E.IdEstado = @IdEstado And Convert( varchar(10), E.FechaRegistro, 120 ) Between @FechaInicial And @FechaFinal --And ClaveLote Like '%*%'
	...
	-------------------------------------------------------	
	If Exists ( Select Name From SysObjects(NoLock) Where Name = 'tmpCteReg_Productos_Compras_Estado' And xType = 'U' )
	  Begin
		Drop Table tmpCteReg_Productos_Compras_Estado
	  End	
	
	Select *
	Into tmpCteReg_Productos_Compras_Estado
	...
	--Select * From tmpCteReg_Productos_Compras_Estado(NoLock) Order By IdFarmacia, DescripcionSal

...
--		Exec spp_Rpt_CteReg_EdoJuris_ListaClavesNegadas_Proceso '21', '*', '*', '2012-04-26', 0, 0, 0, 1, 1 --        


--		Exec spp_Rpt_CteReg_EdoJuris_ListaClavesNegadas_Proceso '21', '*', '2011-09-26', 0, 0, 0, 0     
 	   
 	   
--------------------------------------------------------------------------------------------------------------------- 
If Exists ( Select Name From Sysobjects (NoLock) Where Name = 'spp_Rpt_CteReg_EdoJuris_ListaClavesNegadas_Proceso' and xType = 'P' ) 
   Drop Proc spp_Rpt_CteReg_EdoJuris_ListaClavesNegadas_Proceso 
Go--#SQL 

Create Proc spp_Rpt_CteReg_EdoJuris_ListaClavesNegadas_Proceso 
( 
...
--		spp_Rpt_CteReg_EdoJuris_ListaClavesNegadas_Proceso  
...
--	Select @DiaProceso, @DiasMes 
	...
	Select V.IdEmpresa, V.IdEstado, V.IdFarmacia, V.FolioVenta, 0 as TieneVale  
...
--------------------------------------------- Agregar la informacion Inicial	
	...
	Insert Into ##tmp_Mes ( IdEstado, Estado, IdJurisdiccion, Jurisdiccion, IdFarmacia, Farmacia, Tipo, Año, Mes, NombreMes, ClaveSSA, DescripcionClave, Presentacion, PrecioLicitacion ) 
	Select Distinct IdEstado, @sEstado, IdJurisdiccion, '' as Jurisdiccion, IdFarmacia, Farmacia, Tipo, @Año, @Mes, @NombreMes, ClaveSSA, DescripcionClave, Presentacion, PrecioLicitacion  
...
----------------- Valorizar la sancion 
	If @IdFarmacia = '*' 
	Begin 
		-- Exec spp_Rpt_CteReg_EdoJuris_ListaClavesNegadas_Valorizado @Año, @Mes 	
		Set @sSql = 'Exec spp_Rpt_CteReg_EdoJuris_ListaClavesNegadas_Valorizado  ' + cast(@Año as varchar) + ', ' + cast(@Mes as varchar) 
		Exec(@sSql) 

	 
		Insert Into ##tmp_Mes ( IdEstado, Estado, IdJurisdiccion, Jurisdiccion, IdFarmacia, Farmacia, Tipo, Año, Mes, NombreMes, ClaveSSA, DescripcionClave, Presentacion, PrecioLicitacion ) 
		Select Distinct IdEstado, @sEstado, IdJurisdiccion, '' as Jurisdiccion, IdFarmacia, Farmacia, 2 as Tipo, @Año, @Mes, @NombreMes, ClaveSSA, DescripcionClave, Presentacion, PrecioLicitacion  
		From ##tmpClaves_Negadas 
		-- Where ClaveSSA = '101'  
		Order By DescripcionClave 	
		
		Insert Into ##tmp_Mes ( IdEstado, Estado, IdJurisdiccion, Jurisdiccion, IdFarmacia, Farmacia, Tipo, Año, Mes, NombreMes, ClaveSSA, DescripcionClave, Presentacion, PrecioLicitacion ) 
		Select Distinct IdEstado, @sEstado, IdJurisdiccion, '' as Jurisdiccion, IdFarmacia, Farmacia, 3 as Tipo, @Año, @Mes, @NombreMes, ClaveSSA, DescripcionClave, Presentacion, PrecioLicitacion  
		From ##tmpClaves_Negadas 
		-- Where ClaveSSA = '101'  
		Order By DescripcionClave 		
	End 
...
------ Asignar nombre de Jurisdiccion 	
	...
	If @ProcesoPorDia = 1 
		Begin 
			Set @sSql = 
				...
				'Inner Join ##tmpClaves_Negadas N 
					On (
						M.IdEstado = N.IdEstado and M.IdJurisdiccion = N.IdJurisdiccion and M.IdFarmacia = N.IdFarmacia and 
						M.Año = N.Año and M.Mes = N.Mes and M.ClaveSSA = N.ClaveSSA and N.Dia = ' + cast(@DiaProceso as varchar ) +  ' ) ' +
				'Where M.Tipo = 1 '		 
	...
	Begin 
			...
			Begin 			
				...
				Set @sSql = 
					...
					'Inner Join ##tmpClaves_Negadas N 
						On (
							M.IdEstado = N.IdEstado and M.IdJurisdiccion = N.IdJurisdiccion and  M.IdFarmacia = N.IdFarmacia and 						 
							M.Año = N.Año and M.Mes = N.Mes and M.ClaveSSA = N.ClaveSSA and N.Dia = ' + cast(@iDia as varchar ) +  ' )' + 
				'Where M.Tipo = 1 ' 
				Exec(@sSql) 
				
				--  Mostrar la sancion 
				Set @sSql = 
					'Update M Set _' + cast(@iDia as varchar) + ' = N.ValorSancion '     + char(13) + 
					'From ##tmp_Mes M ' + char(13) + 
					'Inner Join ##tmpClaves_Negadas N 
						On (
							M.IdEstado = N.IdEstado and M.IdJurisdiccion = N.IdJurisdiccion and  M.IdFarmacia = N.IdFarmacia and 						 
				...
				--  Mostrar la sancion 
				Set @sSql = 
					'Update M Set _' + cast(@iDia as varchar) + ' = N.PorcSancion '     + char(13) + 
					'From ##tmp_Mes M ' + char(13) + 
					'Inner Join ##tmpClaves_Negadas N 
						On (
							M.IdEstado = N.IdEstado and M.IdJurisdiccion = N.IdJurisdiccion and  M.IdFarmacia = N.IdFarmacia and 						 
		...
		Set @sSql = 'Update ##tmp_Mes Set Total = ' + @sCampos_Aux + ' Where Tipo in ( 1 ) ' + char(10) + char(13) + 
					'Update ##tmp_Mes Set Sancion = ' + @sCampos_Aux + ' Where Tipo = 2 ' + char(10) + char(13) + 
					'' --- 'Update ##tmp_Mes Set Total = 1 Where Tipo = 3 ' 					
		Exec(@sSql) 	
		
		-- Update T Set Total = Sancion From ##tmp_Mes T Where Tipo = 2 
	End 	
...
---	Select * from ##tmp_Mes 
---	select * from ##tmpClaves_Negadas where ClaveSSA = '101'
...
--- Reporte Acumulado 
	If @ProcesoPorDia = 0 
	Begin 
		Delete From Rpt_CTE_Sanciones_Detalles 
		Where IdEstado = @IdEstado -- and IdJurisdiccion = @IdJurisdiccion 
			  and Año = @Año and Mes = @Mes 
		
		Set @sSql = 
			'Insert Into Rpt_CTE_Sanciones_Detalles ' + -- ( ' + @sCampos_Aux_Base 	+ ' ) ' + 
			'Select ' + @sCampos_Aux_Base_2  + ' ' + 
			'From ##tmp_Mes' 
		-- print @sSql 
		Exec(@sSql) 
...
-------------------------------------------------------	
--- Borrador de proceso 
	If Exists ( Select Name From Sysobjects (NoLock) Where Name = 'Rpt_CTE_Sanciones_Detalles_Aux' and xType = 'U' ) 
	   Drop Table Rpt_CTE_Sanciones_Detalles_Aux 
	   
	Select * 
	Into Rpt_CTE_Sanciones_Detalles_Aux 
	From Rpt_CTE_Sanciones_Detalles    
	Where IdEstado = @IdEstado and Año = @Año and Mes = @Mes 	
-------------------------------------------------------	

--- Integrar resultado de Análisis a procesos terminados 
	If @IntegrarResultado = 1 
	Begin 
		--	Select *  from Rpt_CTE_Sansiones 
		
		Delete From Rpt_CTE_Sansiones 
		Where IdEstado = @IdEstado and Año = @Año and Mes = @Mes 
		
		Insert Into Rpt_CTE_Sansiones 
		Select IdEstado, IdJurisdiccion, IdFarmacia, Año, Mes 
		From Rpt_CTE_Sanciones_Detalles_Aux 
	Where IdEstado = @IdEstado and Año = @Año and Mes = @Mes 			
	End 
	...
	If @MostrarTodo = 0 
		Set @sSql = 'Select ' + @sCampos_Base + ', ' + @sCampos + ', Total  From ##tmp_Mes ' + 
		' Where ( Total > 0 )' + 
...
--- Salida Final 
	Set @sSql = @sSql + '   Select count(distinct ClaveSSA) as Claves, sum(Total) as TotalPiezas From ##tmp_Mes Where ( Tipo = 1 and Total > 0 )  '  	
	--Print @sSql 

...
If Exists ( Select * From sysobjects (nolock) Where Name = 'spp_Rpt_Administrativos_Secretaria_Parte_001' and xType = 'P' ) 
   Drop Proc spp_Rpt_Administrativos_Secretaria_Parte_001 
Go--#SQL


Create Proc spp_Rpt_Administrativos_Secretaria_Parte_001 
(   
...
---- Asignar la farmacia 
	--Set @IdFarmacia = ( Select IdFarmacia From CTE_FarmaciasProcesar ( Nolock ) )

----- Crear Tabla Intermedia 
If Exists ( Select Name From Sysobjects Where Name = 'tmpDatosInicialesLotes_Secretaria' and xType = 'U' )
      Drop Table tmpDatosInicialesLotes_Secretaria
      
Create Table tmpDatosInicialesLotes_Secretaria
(
     ...
     PrecioLicitacion float Null Default 0,
	 ...
	 IdPresentacion_ClaveSSA varchar(3) Not Null Default '',
...
--------- 
	Select Top 0 IdEstado, IdFarmacia 
	Into #CTE_FarmaciasProcesar 
	From CTE_FarmaciasProcesar 

	Insert Into #CTE_FarmaciasProcesar 
...
-- Armar criterios para la consulta 
	...
	Set @sSql = ' 		
	Insert 	Into  tmpDatosInicialesLotes_Secretaria ( IdEstado, -- Estado, ClaveRenapo, IdMunicipio, Municipio, 
		IdFarmacia, 
	...
	From tmpDatosInicialesLotes_Secretaria V (NoLock) 
...
--- Remover los Lotes segun sea el caso 
	If @TipoDispensacion <> 0 
	   Begin 
	      If @TipoDispensacion = 1 
	         Delete From tmpDatosInicialesLotes_Secretaria Where ClaveLote Not Like '%*%'  -- Consignacion 

	      If @TipoDispensacion = 2
	         Delete From tmpDatosInicialesLotes_Secretaria Where ClaveLote Like '%*%'  -- Venta 
	   End 	
	   
	If @TipoInsumo <> 0 
	   Begin 
	      If @TipoInsumo = 1 
	         Delete From tmpDatosInicialesLotes_Secretaria Where TasaIva <> 0  --- Medicamentos 

	      If @TipoInsumo = 2
	         Delete From tmpDatosInicialesLotes_Secretaria Where TasaIva = 0  --- Otros (Material de Curacion, Varios)  
	   End
  
	   
---   spp_Rpt_Administrativos_Secretaria_Parte_001  	
	Set @sSql = '' 
...
---------------------------------------------------------------- 
	...
	Select * 
	Into RptAdmonDispensacion_Secretaria 
	From 
	( 
    ...
    -- Into RptAdmonDispensacion_Secretaria 
	From tmpDatosInicialesLotes_Secretaria (NoLock) 
	...
	Set @sSql = ' 
		Insert Into RptAdmonDispensacion_Secretaria ( IdGrupoPrecios, DescripcionGrupoPrecios, EsSeguroPopular, 
			IdEstado, Estado, ClaveRenapo, IdMunicipio, Municipio, IdColonia, Colonia, Domicilio, 
			IdFarmacia, Farmacia, Folio, FechaSistema, FechaRegistro, 
			IdCliente, NombreCliente, IdSubCliente, NombreSubCliente, 
			IdPrograma, Programa, IdSubPrograma, SubPrograma, 
		...
		'	From tmpDatosInicialesLotes_Secretaria (noLock) ' + char(13) + 
		' ' + @sWhere + char(13) + 			
		    ' Group by ' + char(13) + 
			' IdEstado, Estado, ClaveRenapo, IdMunicipio, Municipio, IdColonia, Colonia, Domicilio, 
			IdFarmacia, Farmacia, Folio, FechaSistema, FechaRegistro, 

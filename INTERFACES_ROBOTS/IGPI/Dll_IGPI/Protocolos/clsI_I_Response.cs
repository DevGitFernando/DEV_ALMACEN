...
namespace Dll_IGPI.Protocolos
{
    public class clsI_I_Response
    {
        ...
        IGPI_I_OrdersState I_Estado_Respuesta = IGPI_I_OrdersState.None; 
        ...
        public clsI_I_Response(IGPIParametros Parametros)
        {
            sRespuesta = ""; 
            pMach = Parametros; 
            pMach.Dialogo = "I"; 
        ...
        private void DecodificarMensaje()
        {
            ...
            ////    " right( (replicate('0', {1}) + CodigoEAN), {1}) as CodigoDll_IGPI, * " +
            ...
            sSql = string.Format("Select top 1 " +
                ...
                " ERP_Date, IdProducto, CodigoEAN, Descripcion, ClaveSSA, DescripcionClave, Status, StatusAux, StatusIGPI, StatusIGPIAux " + 
                " From vw_IGPI_CFGC_Productos (NoLock) " +
                " Where right( (replicate('0', {1}) + CodigoEAN), {1}) = '{0}' ", Fg.PonCeros(sCodigoEAN, iNumEAN), iNumEAN);


            i_Estado = (IGPI_i_State)(Convert.ToInt32(pMach.State.Trim()));
            I_Estado_Respuesta = IGPI_I_OrdersState.NotAllowToStore;

            if ((i_Estado == IGPI_i_State.ConfirmationPackageStored || i_Estado == IGPI_i_State.ConfirmationPackageNotStored))
            {
                bResponder = false;
                if (i_Estado == IGPI_i_State.ConfirmationPackageStored)
                {
                    sSql = string.Format("Exec spp_Mtto_IGPI_StockProductos @CodigoEAN = '{0}', @Cantidad = '{1}', @Status = 'A', @Tipo = '1', @Proceso = '{2}' ",
                   ...
                   i_Estado == IGPI_i_State.EntryOfGoods
                || i_Estado == IGPI_i_State.InternalRearrangementsOfGoods
                || i_Estado == IGPI_i_State.StartNewDelivery
                || i_Estado == IGPI_i_State.EndOfDelivery
                || i_Estado == IGPI_i_State.EntryOfGoodsWithMaintenanceOfTheBachCoding
                || i_Estado == IGPI_i_State.InternalRearrangementOfGoodsWithMaintenanceOfBachCoding)
            {

                I_Estado_Respuesta = IGPI_I_OrdersState.AllowToStore;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            // Final 
                            I_Estado_Respuesta = (IGPI_I_OrdersState)(Convert.ToInt32(leer.Campo("StatusIGPI")));
                            pMach.OrdersState = leer.Campo("StatusIGPI");
                        }
                    }
            ...
            sRespuesta = pMach.Dialogo + pMach.OrderNumber + pMach.RequestLocationNumber + 
                ...
                pMach.ProductCode + pMach.Quantity + pMach.Expiry_Date + pMach.OrdersState + 
        ...
        private string Respuesta_A_Peticion(IGPI_I_OrdersState TipoRespuesta)
        {
            string sRegresa = "";

            switch (TipoRespuesta)
            {
                case IGPI_I_OrdersState.AllowToStore:
                    sRegresa = "Ingreso permitido.";
                    break;

                case IGPI_I_OrdersState.AllowToStoreWithUseByDate:
                    sRegresa = "Ingreso permitido con caducidades";
                    break;

                case IGPI_I_OrdersState.None:
                    sRegresa = "Ninguna.";
                    break;

                case IGPI_I_OrdersState.NoSettingOfMaintenanceOfBachCode:
                    sRegresa = "Sin instrucciones de mantenimiento de codigos";
                    break;

                case IGPI_I_OrdersState.NotAllowToStore:
                    sRegresa = "Ingreso no permitido.";
                    break;

                case IGPI_I_OrdersState.StoreProductoInFridge:
        ...
        private string Respuesta_A_Peticion(IGPI_i_State TipoPeticion)
        {
            string sRegresa = "";

            switch (TipoPeticion)
            {
                case IGPI_i_State.ConfirmationPackageNotStored:
                    sRegresa = "Paquete no almacenado.";
                    break;

                case IGPI_i_State.ConfirmationPackageStored:
                    sRegresa = "Paquete almacenado.";
                    break;

                case IGPI_i_State.EndOfDelivery:
                    sRegresa = "Entrega finalizada.";
                    break;

                case IGPI_i_State.EntryOfGoods:
                    sRegresa = "Ingreso permitido.";
                    break;

                case IGPI_i_State.EntryOfGoodsWithMaintenanceOfTheBachCoding:
                    sRegresa = "Ingreso permitido con mantenimiento de codigos.";
                    break;

                case IGPI_i_State.InternalRearrangementOfGoodsWithMaintenanceOfBachCoding:
                    sRegresa = "Reingreso de mercancia permitida con mantenimiento de codigos.";
                    break;

                case IGPI_i_State.InternalRearrangementsOfGoods:
                    sRegresa = "Movimiento interno de stock.";
                    break;

                case IGPI_i_State.None:
                    sRegresa = "Ninguna.";
                    break;

                case IGPI_i_State.StartNewDelivery:

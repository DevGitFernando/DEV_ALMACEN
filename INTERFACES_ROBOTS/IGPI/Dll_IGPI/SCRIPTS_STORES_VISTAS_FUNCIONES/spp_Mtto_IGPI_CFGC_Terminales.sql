If Exists (Select Name From SysObjects(NoLock) Where Name = 'spp_Mtto_IGPI_CFGC_Terminales' and xType = 'P')
    Drop Proc spp_Mtto_IGPI_CFGC_Terminales
Go--#SQL  
  
Create Proc spp_Mtto_IGPI_CFGC_Terminales 
( 
...
Declare @sMensaje varchar(1000), 
	...
	*/
	...
	If @IdTerminal = '*' 
	   Select @IdTerminal = cast( (max(IdTerminal) + 1) as varchar)  From IGPI_CFGC_Terminales (NoLock) 
	...
	If @iOpcion = 1 
       Begin 
		   ...
		   If Not Exists ( Select * From IGPI_CFGC_Terminales (NoLock) Where IdTerminal = @IdTerminal ) 
			  Begin 
				 Insert Into IGPI_CFGC_Terminales ( IdTerminal, Nombre, MAC_Address, EsDeSistema, Status, Actualizado ) 
				 Select @IdTerminal, @Nombre, @MAC_Address, @iEsInterface, @sStatus, @iActualizado 
              End 
		   Else 
			  Begin 
			     Update IGPI_CFGC_Terminales 
    ...
    Else 
       Begin 
           Set @sStatus = 'C' 
	       Update IGPI_CFGC_Terminales Set Status = @sStatus, Actualizado = @iActualizado Where IdTerminal = @IdTerminal 

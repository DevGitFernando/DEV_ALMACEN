If Exists( Select Name From SysObjects(NoLock) Where Name = 'vw_Productos_IGPI' And xType = 'V' )
	Drop view vw_Productos_IGPI
Go--#SQL  

Create view vw_Productos_IGPI
As 
/*	
...
*/
	...
	Select (case when IsNull(IP.IdProducto, 0) = 0 then 0 else 1 end) as EsGPI, 
	    IsNull(EP.ExistenciaIGPI, 0) as ExistenciaIGPI, 
    ...
    Left Join IGPI_CFGC_Clientes_Productos IP (NoLock) On ( P.IdProducto = IP.IdProducto and P.CodigoEAN = IP.CodigoEAN and IP.Status = 1 ) 
	Left Join IGPI_StockProductos EP (NoLock) On ( IP.CodigoEAN = EP.CodigoEAN ) 
Go--#SQL  

--	sp_listacolumnas vw_Productos_CodigoEAN 

--	select * from vw_Productos_IGPI   

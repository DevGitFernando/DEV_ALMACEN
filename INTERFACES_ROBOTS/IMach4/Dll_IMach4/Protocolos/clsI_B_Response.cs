...
namespace Dll_IMach4.Protocolos
{
    public class clsI_B_Response
    {
        ...
        private void DecodificarMensaje(string Mensaje)
        {
            ...
            pMach.StorageLocation = pMach.Cortar(ref sMsj, 16);
            ...
            pMach.ERP_Date = pMach.Cortar(ref sMsj, 8);
            ...
            ////string sSql	= string.Format(" update IMach_StockProductos set ExistenciaIMach = '{2}', Actualizado = 0 " + 
            ////    " Where right( (replicate('0', {1}) + CodigoEAN), {1}) = '{0}' ", 
            ////    Fg.PonCeros(sCodigoEAN, iNumEAN), iNumEAN, pMach.Quantity); 

            string sSql = string.Format("Exec spp_Mtto_IMach_StockProductos @CodigoEAN = '{0}', @Cantidad = '{1}', @Status = 'A', @Tipo = '1', @Proceso = '{2}' ",
